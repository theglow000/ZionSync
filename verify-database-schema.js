      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        if (fieldString) {
          ...
          const fieldNames = fieldString
            ...
            .filter(field => !['$exists', '$gte', '$lte', '$in', '$ne'].includes(field));
...
function analyzeModelFiles() {
  ...
  const modelFiles = fs.readdirSync(MODELS_DIR, { withFileTypes: true })
  ...
  modelFiles.forEach(filePath => {
    try {
      ...
      while ((match = schemaRegex.exec(content)) !== null) {
        ...
        findings.collections.get(collectionName).fields.add(`${fieldName}:${fieldType}`);
      }
    } catch (error) {
...
function parseSchemaDocumentation() {
  try {
    ...
    while ((match = codeBlockRegex.exec(content)) !== null) {
      ...
      const lastCollectionMatch = [...beforeBlock.matchAll(/####\s+(\w+)\s*$/gm)].pop();
      ...
      if (currentCollection && documented.fields.has(currentCollection)) {
        ...
        while ((fieldMatch = fieldRegex.exec(codeBlock)) !== null) {
          ...
          documented.fields.get(currentCollection).add(`${fieldName}:${fieldType.trim()}`);
        }
        ...
        while ((dynamicMatch = dynamicFieldRegex.exec(codeBlock)) !== null) {
          ...
          documented.fields.get(currentCollection).add(`${fieldPattern}:${fieldType.trim()}`);
        }
        ...
        while ((spreadMatch = spreadRegex.exec(codeBlock)) !== null) {
          ...
          documented.fields.get(currentCollection).add(`...${spreadName}:${fieldType.trim()}`);
        }
        ...
        while ((quotedMatch = quotedFieldRegex.exec(codeBlock)) !== null) {
          ...
          console.log(`Found quoted field: "${fieldName}" in collection ${currentCollection}`);
          documented.fields.get(currentCollection).add(`${fieldName}:${fieldType.trim()}`);
        }
      }
    }
...
function compareSchemas(actualCollections, documentedSchema) {
  ...
  for (const [collName, collection] of actualCollections) {
    if (documentedSchema.collections.includes(collName)) {
      ...
      const documentedFields = documentedSchema.fields.get(collName) || new Set();
      ...
      for (const field of actualFields) {
        ...
        const isDocumented = Array.from(documentedFields).some(docField => {
          ...
          if (docFieldName.includes('${') && fieldName.includes('team_member_')) {
            return true;
          }
          
          // Dynamic pattern match for selections.${position} patterns  
          if (docFieldName.includes('selections.${position}') && fieldName.includes('selections.')) {
            return true;
          }
...
function generateReport() {
  ...
  Array.from(findings.collections.entries())
    ...
    .forEach(([name, collection]) => {
      console.log(`   📁 ${name}`);
      console.log(`      Operations: ${Array.from(collection.operations).join(', ')}`);
      if (collection.fields.size > 0) {
        const fieldList = Array.from(collection.fields).slice(0, 10).join(', ');
        const moreFields = collection.fields.size > 10 ? ` ... (+${collection.fields.size - 10} more)` : '';
  ...
  if (findings.discrepancies.length > 0) {
    ...
    findings.discrepancies.forEach((discrepancy, index) => {
      console.log(`   ${index + 1}. ${discrepancy.type}: ${discrepancy.collection}`);
  ...
  console.log(`   Status: ${findings.discrepancies.length === 0 ? '✅ PASSED' : '❌ FAILED'}`);
  console.log(`   Accuracy: ${Math.round((1 - (findings.discrepancies.length / Math.max(findings.collections.size, 1))) * 100)}%`);
}
...
async function main() {
  ...
  console.log(`   Found ${documentedSchema.collections.length} documented collections`);
  
  // Step 6: Compare schemas
  console.log('🔍 Comparing schemas...');
  findings.discrepancies = compareSchemas(findings.collections, documentedSchema);
...
module.exports = { extractDatabaseOperations, analyzeModelFiles, parseSchemaDocumentation, compareSchemas };
