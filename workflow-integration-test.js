...
class WorkflowIntegrationTester {
  ...
  async testAPIMergeLogic() {
    this.log(
      "📋 Test 1: Verifying API merge logic in service-details route",
      "test",
    );
    ...
    const criticalChecks = [
      {
        ...
        check: () =>
          apiRoute.includes("CRITICAL FIX: Preserve existing song selections"),
      ...
      },
      {
        ...
        check: () =>
          apiRoute.includes("existingSongSelections.set(prefix, {") &&
      ...
      },
      {
        ...
        check: () =>
          apiRoute.includes("elements: mergedElements") &&
          apiRoute.includes(
            "Use merged elements that preserve song selections",
          ),
    ...
    for (const check of criticalChecks) {
      if (check.check()) {
        this.log(
          `  ✅ ${check.name}: PASSED - ${check.description}`,
          "success",
        );
        passed++;
      } else {
    ...
    this.log(
      `📊 API Test Results: ${passed}/${criticalChecks.length} checks passed`,
      passed === criticalChecks.length ? "success" : "error",
    );

    if (failed > 0) {
      throw new Error(
        `API merge logic test failed: ${failed} critical checks failed`,
      );
    }
  }
  ...
  async testSignupSheetMergeLogic() {
    this.log(
      "📋 Test 2: Verifying SignupSheet merge logic in onSave handler",
      "test",
    );
    ...
    const criticalChecks = [
      {
        ...
        check: () =>
          signupSheet.includes("💾 PASTOR SAVE: Starting save process"),
      ...
      },
      {
        ...
        check: () =>
          signupSheet.includes(
            "const existingSongs = existingElements.filter(e => e.type === 'song_hymn'",
      ...
      },
      {
        ...
        check: () =>
          signupSheet.includes(
            "const newElementPrefix = newElement.content?.split(",
          ) &&
          signupSheet.includes(
            "const existingPrefix = existing.content?.split(",
          ),
        description:
          "Confirms intelligent prefix matching for song preservation",
      },
      {
        name: "Positional fallback matching",
        check: () =>
          signupSheet.includes("try positional matching") &&
          signupSheet.includes(
      ...
      },
      {
        ...
        description:
          "Verifies proper content reconstruction with preserved selections",
      },
      {
        name: "Reading reference preservation",
        check: () =>
          signupSheet.includes("Handle readings with exact prefix matching") &&
    ...
    this.log(
      `📊 SignupSheet Test Results: ${passed}/${criticalChecks.length} checks passed`,
      passed === criticalChecks.length ? "success" : "error",
    );

    if (failed > 0) {
      throw new Error(
        `SignupSheet merge logic test failed: ${failed} critical checks failed`,
      );
    }
  }
...
Dismissal`,
      elements: [
        ...
        },
        {
          id: "elem_3",
          type: "liturgy",
          content: "Confession and Forgiveness (pg. 94-96)",
        },
        {
          id: "elem_4",
          type: "reading",
    ...
    const serviceWithSongs = {
      ...
      elements: [
        ...
        },
        {
          ...
          type: "message",
          content: "Sermon:",
          reference: "The Light of the World",
        },
        {
          id: "elem_7",
          type: "song_hymn",
        ...
        },
        {
          ...
          type: "song_hymn",
          content:
            "Sending Song: Go My Children, with My Blessing #721 (Cranberry)",
          selection: {
            type: "hymn",
...
Dismissal`;
    ...
    const mergedElements = this.simulateSignupSheetMerge(
      pastorEditedElements,
      serviceWithSongs.elements,
    );

    // Verify song selections were preserved
    const preservedSongs = mergedElements.filter(
      (el) =>
    ...
    this.log(
      `🎵 Songs with preserved selections: ${preservedSongs.length}`,
      "info",
    );
    ...
    const preservedReadings = mergedElements.filter(
      (el) => el.type === "reading" && el.reference,
    );

    this.log(
      `📖 Readings with preserved references: ${preservedReadings.length}`,
      "info",
    );
    ...
    const apiMergedElements = this.simulateAPIMerge(
      pastorEditedElements,
      serviceWithSongs.elements,
    );
    const apiPreservedSongs = apiMergedElements.filter(
      (el) =>
        (el.type === "song_hymn" || el.type === "song_contemporary") &&
        el.selection?.title,
    );
    ...
    if (preservedSongs.length !== apiPreservedSongs.length) {
      throw new Error(
        `Merge strategy mismatch: SignupSheet preserved ${preservedSongs.length} songs, API preserved ${apiPreservedSongs.length} songs`,
      );
    }
    ...
    const assertions = [
      {
        ...
        check: () =>
          preservedSongs.some((s) => s.content.includes("Holy, Holy, Holy")),
        error: 'Opening Hymn "Holy, Holy, Holy" was not preserved',
      },
      {
        name: "Hymn of the Day preserved",
        check: () =>
          preservedSongs.some((s) => s.content.includes("Amazing Grace")),
        error: 'Hymn of the Day "Amazing Grace" was not preserved',
      },
      {
        name: "Sending Song preserved",
        check: () =>
          preservedSongs.some((s) => s.content.includes("Go My Children")),
        error:
          'Sending Song "Go My Children, with My Blessing" was not preserved',
      },
      {
        name: "First Reading reference preserved",
        check: () =>
          preservedReadings.some((r) => r.reference === "Isaiah 6:1-8"),
        error: 'First Reading reference "Isaiah 6:1-8" was not preserved',
      },
      {
        name: "Gospel Reading reference preserved",
        check: () =>
          preservedReadings.some((r) => r.reference === "John 1:1-14"),
    ...
    this.log(
      `📊 Workflow Test Results: ${assertionsPassed}/${assertions.length} assertions passed`,
      assertionsFailed === 0 ? "success" : "error",
    );

    if (assertionsFailed > 0) {
      throw new Error(
        `Workflow simulation failed: ${assertionsFailed} critical assertions failed`,
      );
    }

    this.log(
      "✅ Complete workflow simulation PASSED - Song selections preserved after pastor edit!",
      "success",
    );
  }
  ...
  simulateSignupSheetMerge(newElements, existingElements) {
    const existingSongs = existingElements.filter(
      (e) =>
        (e.type === "song_hymn" || e.type === "song_contemporary") &&
        e.selection?.title,
    );
    const existingReadings = existingElements.filter(
      (e) => e.type === "reading" && e.reference,
    );
    ...
    return newElements.map((newElement) => {
      ...
      if (
        ...
        const newElementPrefix = newElement.content
          ?.split(":")[0]
          ?.trim()
          .toLowerCase();
        ...
        let matchingSong = existingSongs.find((existing) => {
          const existingPrefix = existing.content
            ?.split(":")[0]
            ?.trim()
            .toLowerCase();
        ...
        if (!matchingSong) {
          const newSongElements = newElements.filter(
      ...
      if (
        ...
        if (existingSelection) {
          ...
          if (selection.type === "hymn") {
            const hymnalName =
              selection.hymnal.charAt(0).toUpperCase() +
              selection.hymnal.slice(1);
  ...
  generateReport() {
    ...
    if (errorCount === 0) {
      ...
      this.log(
        "❌ TESTS FAILED! The workflow fix has issues that need to be addressed.",
        "error",
      );
      this.log(
        `📊 Results: ${successCount} passed, ${errorCount} failed out of ${totalTests} total`,
        "error",
      );
    }
