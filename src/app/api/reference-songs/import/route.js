...
export async function POST(request) {
  try {
    ...
    if (!existingSong && referenceSong.type === 'hymn' && referenceSong.number && referenceSong.hymnal) {
      existingSong = await db.collection("songs").findOne({
        type: 'hymn',
    ...
    if (existingSong) {
      ...
      const newSong = {
        ...
        hymnaryLink: referenceSong.type === 'hymn' && referenceSong.number && referenceSong.hymnal 
    ...
    const serviceDetails = await db.collection("serviceDetails").findOne({ date: serviceDate });
    ...
    updatedElements[elementIndex] = {
      ...
      reference: existingSong.type === 'hymn' && existingSong.number && existingSong.hymnal
        ? `#${existingSong.number} - ${existingSong.hymnal}`
        : existingSong.author ? `by ${existingSong.author}` : ''
    };
    ...
    const positionMatch = songPositionMappings.find(m => m.positionId === position);
    const positionNumber = positionMatch ? parseInt(position.replace('song_', ''), 10) : -1;
