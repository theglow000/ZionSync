...
export async function POST(request) {
  try {
      ...
      { title: validatedData.title },
      {
        $setOnInsert: {
          ...
          songData: {
            ...
            author: body.author,
            sheetMusic:
              body.type === "hymn" ? body.hymnaryLink : body.songSelectLink,
    ...
    await db
      .collection("songs")
      .updateOne({ title: validatedData.title }, { $inc: { usageCount: 1 } });
...
async function updateSongRotationStatus(title, db) {
  try {
    ...
    const sortedUses = [...songUsage.uses].sort(
    ...
    const rotationScore = Math.min(
      10,
      Math.ceil((recentUses / recentServicesWindow) * 10),
    );
