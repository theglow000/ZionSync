...
} from "@/lib/LiturgicalCalendarService";
...
export async function GET() {
  try {
    ...
    const currentSeasonSongs = songs.filter(
      (song) =>
        song.seasonalTags &&
        song.seasonalTags.includes(LITURGICAL_SEASONS[currentSeasonId].name),
    );
    const currentSeasonReadiness = {
      ...
      highComfortSongs: currentSeasonSongs.filter(
        (song) => (songComfortMap[song.title] || 0) >= 7,
      ).length,
      daysRemaining: daysRemaining,
      readinessScore:
        currentSeasonSongs.length > 0
          ? Math.round(
              (currentSeasonSongs.filter(
                (song) => (songComfortMap[song.title] || 0) >= 7,
              ).length /
                currentSeasonSongs.length) *
                100,
            )
          : 0,
    };
    ...
    const nextSeasonSongs = songs.filter(
      (song) =>
        song.seasonalTags &&
        song.seasonalTags.includes(LITURGICAL_SEASONS[nextSeasonId].name),
    );
    const nextSeasonReadiness = {
      ...
      highComfortSongs: nextSeasonSongs.filter(
        (song) => (songComfortMap[song.title] || 0) >= 7,
      ).length,
      daysRemaining: daysRemaining,
      readinessScore:
        nextSeasonSongs.length > 0
          ? Math.round(
              (nextSeasonSongs.filter(
                (song) => (songComfortMap[song.title] || 0) >= 7,
              ).length /
                nextSeasonSongs.length) *
                100,
            )
          : 0,
    };
