...
export function LiturgicalDebugger() {
  ...
  function getExpectedSeason(label) {
    switch(label) {
      case "Ash Wednesday": return "LENT";
      case "Easter Sunday": return "EASTER";
      case "Advent 1": return "ADVENT";
      case "All Saints": return "ALL_SAINTS"; 
      case "Transfiguration": return "EPIPHANY"; // Last Sunday of Epiphany
      case "Palm Sunday": return "HOLY_WEEK"; // Palm Sunday starts Holy Week
      case "Christmas Eve": return "CHRISTMAS";
    ...
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-4">
        <p className="text-sm text-gray-600">
          Testing boundary transitions between liturgical seasons. 
          These dates should mark the beginning of their respective seasons.
        </p>
      </div>
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date Label</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Calculated Season</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Expected Season</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Special Day</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Key Date</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-200">
          {results.map((result, index) => (
            <tr key={index} className={result.season === result.expectedSeason ? "bg-green-50" : "bg-red-50"}>
              <td className="px-4 py-2 whitespace-nowrap text-sm font-medium text-gray-900">{result.label}</td>
              <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{result.date}</td>
              <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{result.season || "undefined"}</td>
              <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{result.expectedSeason}</td>
              <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{result.specialDay || "none"}</td>
