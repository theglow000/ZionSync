...
const SongRediscoveryPanel = () => {
      ...
      try {
        ...
        if (liturgicalInfo && liturgicalInfo.season) {
          ...
          if (typeof liturgicalInfo.season === "string") {
            ...
            seasonKey =
              Object.keys(LITURGICAL_SEASONS).find(
  ...
  const fetchIntelligentSuggestions = async () => {
    try {
      ...
      const response = await fetchWithTimeout(
        `/api/song-usage/suggestions?${params}`,
      );
  ...
  const getFilteredSongs = () => {
    return songs.filter((song) => {
      ...
      const matchesSeason =
        selectedSeason === "all" ||
        (song.seasonalTags &&
          song.seasonalTags.some(
            (tag) => tag.toLowerCase() === selectedSeason.toLowerCase(),
          ));

      // Timeframe filter - using the analytics data
      let matchesTimeframe = true;
      if (timeframeFilter !== "all") {
        const usageInfo = usageAnalytics.frequency.find(
          (u) => u.title === song.title,
        );
        ...
        if (timeframeFilter === "unused" && hasBeenUsed) {
          ...
          timeframeFilter === "used-once" &&
          (!hasBeenUsed || (usageInfo && usageInfo.count > 1))
        ) {
          matchesTimeframe = false;
        }
      }
  ...
  const getCategorizedSongs = () => {
    ...
    const unusedSongs = filteredSongs.filter(
      (song) => !songUsage[song.title] || songUsage[song.title] === 0,
    );
    const rarelyUsedSongs = filteredSongs.filter(
      (song) => songUsage[song.title] === 1,
    );
    ...
    if (currentSeason) {
      ...
      seasonalSongs[currentSeasonId] = filteredSongs.filter(
        (song) =>
          song.seasonalTags &&
          song.seasonalTags.some(
            (tag) => tag.toLowerCase() === currentSeasonId.toLowerCase(),
          ),
      );
    }
  ...
  const getUpcomingSeasonInfo = () => {
    ...
    if (upcomingSuggestion?.liturgicalContext?.upcomingSeason) {
      return {
        ...
        daysUntil:
          upcomingSuggestion.liturgicalContext.upcomingSeason.daysUntil,
        songs: intelligentSuggestions.filter(
          (song) =>
  ...
  const formatSongInfo = (song) => {
    if (song.type === "hymn") {
      return song.number
        ? `#${song.number} ${song.hymnal ? `(${song.hymnal})` : ""}`
        : "";
    } else {
      return song.author || "";
    }
  ...
  const getSongUsageCount = (songTitle) => {
    const usage = usageAnalytics.frequency.find(
  ...
  const getSeasonName = (seasonId) => {
    const season = Object.entries(LITURGICAL_SEASONS).find(
      ([key, season]) => key.toLowerCase() === seasonId.toLowerCase(),
    );
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden flex flex-col h-full">
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
                  ...
                  {isSuggestionsLoading ? (
                    ...
                    <div>
                      <p className="text-xs text-gray-500 mb-2 px-2">
                        These suggestions prioritize seasonal appropriateness
                        and songs you haven't used in a while.
                      </p>
                          ...
                          >
                            <div>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {song.liturgicalContext?.seasonMatches?.some(
              ...
              return (
                <div className="border rounded-md overflow-hidden mb-6">
                    ...
                    <div className="flex items-center">
                      <h3 className="font-medium text-amber-800">
                        ...
                        <span className="ml-2 text-xs">
                          ({upcomingInfo.daysUntil}{" "}
                          {upcomingInfo.daysUntil === 1 ? "day" : "days"} away)
                        </span>
                      </h3>
                    </div>
                  ...
                  {expandedSections.upcoming && (
                    <div className="p-2">
                      <p className="text-xs text-gray-500 mb-2 px-2">
                        Start preparing songs for the upcoming liturgical
                        season.
                      </p>
                      <ul className="divide-y divide-gray-100">
            ...
            <div className="border rounded-md overflow-hidden">
              ...
              {expandedSections.seasonal && (
                <div className="divide-y divide-gray-100">
                  {Object.entries(categorizedSongs.seasonal).map(
                    ([season, seasonSongs]) => {
                        ...
                        <div key={season} className="p-2">
                          <h4 className="text-sm font-medium text-gray-700 px-2 py-1 flex items-center">
                            <span
                              ...
                              style={{
                                backgroundColor:
                                  Object.entries(LITURGICAL_SEASONS).find(
                                    ([key]) =>
                                      key.toLowerCase() ===
                                      season.toLowerCase(),
                                  )?.[1]?.color || "#888888",
                            ...
                            {seasonSongs.length > 5 && (
                              <li className="p-2 text-center">
                                <span className="text-xs text-blue-600">
                                  + {seasonSongs.length - 5} more songs for this
                                  season
                                </span>
                              </li>
                            )}
                                      ...
                                      {position.hasSelection ? (
                                        <div className="flex items-center gap-1 text-xs text-gray-500 mr-2">
                                          ...
                                          <span className="font-medium truncate max-w-[120px]">
                                            {position.selectionDetails.title}
                                          </span>
                                        </div>
                                      ) : null}
      ...
      {alert.show && (
        <div
          className={`fixed bottom-4 right-4 p-3 rounded-md shadow-md z-50 ${
            alert.type === "success"
