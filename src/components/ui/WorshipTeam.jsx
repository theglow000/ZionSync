...
const WorshipTeam = ({
    ...
    const fetchTeamsAndAssignments = async () => {
      try {
        ...
        const assignmentsResponse = await fetchWithTimeout(
          API_ENDPOINTS.WORSHIP_ASSIGNMENTS,
          {
            signal: controller.signal,
          },
        );
        ...
        assignmentsData.forEach((assignment) => {
          if (assignment.date) {
            ...
            const hasSongs = service?.elements?.some(
  ...
  const handleSongSelection = useCallback(
    async (date, songId, songData) => {
      ...
      try {
        ...
        handleError(
          error,
          { operation: "selectSong", date, songId },
          ERROR_MESSAGES.SONG_SELECT_ERROR,
        );
      } finally {
        // Clear pending state
  ...
  const getSeasonInfo = useCallback(
    (date) => {
      ...
      if (!service?.liturgical)
        return { name: "", color: "", hasSpecialDay: false };
                            ...
                            header={
                              ...
                              >
                                ...
                                <div className="flex items-center gap-3 flex-shrink-0">
                                  ...
                                  {serviceDetails[item.date]?.elements?.some(
                                    (e) =>
                                      e.type === "song_hymn" &&
                                      e.selection?.title,
                                  ) && (
