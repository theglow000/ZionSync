...
const SignupSheet = ({
  ...
  const checkForOrderOfWorship = useCallback(
    (date) => {
  ...
  const handleRemoveUser = useCallback(
    async (userName, skipConfirm = true) => {
      if (!skipConfirm) {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const [
          ...
          fetchWithTimeout(API_ENDPOINTS.SIGNUPS, {
            signal: controller.signal,
          }),
          fetchWithTimeout(API_ENDPOINTS.COMPLETED, {
            signal: controller.signal,
          }),
          fetchWithTimeout(API_ENDPOINTS.CUSTOM_SERVICES, {
            signal: controller.signal,
          }),
        ]);
        ...
        // Parse responses
        const [usersData, signupsData, completedData, customServicesData] =
          await Promise.all([
            usersResponse.json(),
              ...
              Object.keys(dataObj).forEach((date) => {
                ...
                existingElements.forEach((element) => {
                  ...
                  if (
                    ...
                    const songIndex = existingElements
                      .filter(
                        (e) =>
                          e.type === element.type ||
                          (e.type === "song_hymn" &&
                            element.type === "song_contemporary") ||
                          (e.type === "song_contemporary" &&
                            element.type === "song_hymn"),
                      )
                      .indexOf(element);
                ...
                const mergedElements = newElements.map((newElement, index) => {
                  ...
                  // 2. Try matching by content prefix
                    ...
                    const contentPrefix = newElement.content
                      ?.split(":")[0]
                      ?.trim();
                    if (contentPrefix) {
                  ...
                  if (
                    ...
                    if (
                      songIndex >= 0 &&
                      existingElementMap.has(`song-${songIndex}`)
                    ) {
                      existingElement = existingElementMap.get(
                        `song-${songIndex}`,
                      );
                    }
                  }
                  ...
                  if (existingElement) {
                    ...
                    if (
                      newElement.type === "reading" ||
                      newElement.type === "message"
                    ) {
                      return {
                        ...existingElement,
                        ...newElement,
  ...
  const handleSignup = useCallback(async () => {
    ...
    try {
      ...
      handleSuccess(
        "Successfully signed up! Date added to calendar selection.",
      );
    } catch (error) {
  ...
  const handleServiceDetailChange = useCallback(
  ...
  const handleCompleted = useCallback(
    async (date) => {
      ...
      try {
        ...
        handleError(
          error,
          { operation: "updateCompletion", date },
          ERROR_MESSAGES.COMPLETION_ERROR,
        );
      } finally {
        // Clear pending state
  ...
  const handleDeleteServiceDetails = useCallback(
    async (date) => {
      const confirmed = await confirm({
  ...
  const checkForSelectedSongs = useCallback(
    (date) => {
      const elements = serviceDetails[date]?.elements;
      const songElements = elements?.filter(
        (element) => element.type === "song_hymn",
      );

      // Check if any songs have a reference (which indicates they were selected)
      // OR if they have a selection.title
      return songElements?.some(
        (element) => element.reference || element.selection?.title,
      );
    },
    [serviceDetails],
  );
  ...
  const handleAssignUser = useCallback(
    async (date, userName) => {
      ...
      const [month, day, shortYear] = date
        .split("/")
        .map((num) => parseInt(num, 10));
      ...
      try {
        ...
        handleError(
          error,
          { operation: "assignUser", date, userName },
    ...
    [signups, signupDetails, selectedDates, handleSuccess, handleError],
  ); // Add error handlers
    ...
    >
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="sticky top-0 z-10 bg-white">
          <CardHeader className="border-b border-gray-200">
            ...
            <div className="hidden md:flex items-center justify-center gap-12">
              ...
              />
              <div>
                <h1 className="text-3xl font-bold text-center text-[#6B8E23]">
                  Proclaim Presentation Team
                </h1>
            ...
            <div className="flex md:hidden flex-col gap-4">
              ...
              <div className="text-center">
                <h1 className="text-xl font-bold text-[#6B8E23]">
                  Proclaim Presentation Team
                </h1>
                <div className="flex items-center justify-center mt-2">
                  <YearSelector
                    ...
                    <div className="relative group">
                      ...
                      <div className="absolute left-0 top-full mt-2 w-72 bg-white p-3 rounded-lg shadow-lg invisible group-hover:visible z-50 text-sm border border-gray-200 text-slate-900">
                        ...
                        <ol className="list-decimal ml-4">
                          ...
                          <li className="text-slate-900">
                            Outlook will open automatically
                          </li>
                          <li className="text-slate-900">
                            Click "Save & Close"
                          </li>
        ...
        <div className="flex-1 overflow-hidden">
          {" "}
          {/* This wrapper prevents double scrollbar */}
              ...
              <LoadingSpinner
                message={
                  datesLoading
                    ...
                    : "Loading Presentation Team schedule..."
                }
                color="[#6B8E23]"
              />
            ) : dates.length === 0 ? (
                      ...
                      <MobileServiceCard
                        ...
                        onDeleteService={async (date) => {
                          try {
                            ...
                            console.error(
                              "Error deleting service details:",
                              error,
                            );
                            handleError(
                              error,
                      ...
                      <table className="w-full">
                        <thead className="sticky top-0 z-10">
                          <tr className="bg-[#FFF8DC]">
                            ...
                            <th
                              style={{ width: "96px" }}
                              className="p-2 text-center font-bold text-[#6B8E23]"
                            >
                              Completed
                            </th>
                          </tr>
                                ...
                                <tr>
                                  <td colSpan="7" className="p-2 bg-gray-50">
                                    <div className="space-y-0 ml-[10%] max-w-xl">
                                      ...
                                      {serviceDetails[item.date]?.elements?.map(
                                        (element, index) => (
                                          <div
                                            key={index}
                                      ...
                                      {(!serviceDetails[item.date]?.elements ||
                                        serviceDetails[item.date]?.elements
                                          .length === 0) && (
          ...
          onSave={async (serviceData) => {
            try {
              ...
              const existingElements =
                serviceDetails[editingDate]?.elements || [];
              ...
              const updatedElements = serviceData.elements.map((newElement) => {
                ...
                existingElements.forEach((existing) => {
                  ...
                  if (
                    usedExistingElements.has(existing.id) ||
                  ...
                  const existingPrefix = existing.content
                    ...
                    .trim()
                    .toLowerCase();
                  const newPrefix = newElement.content
                    .split(":")[0]
                    .trim()
                    .toLowerCase();
                  ...
                  if (existingPrefix === newPrefix) {
                    ...
                    existingPrefix.includes(newPrefix) ||
                    newPrefix.includes(existingPrefix)
                  ) {
                    score += 20;
                  }

                  // STRATEGY 3: Position similarity (20 points max)
                  const existingIndex = existingElements.findIndex(
                    (el) => el.type === existing.type,
                  );
                  const newIndex = serviceData.elements.findIndex(
                    (el) => el.type === newElement.type,
                  );
                  ...
                  if (
                    existing.type === "song_hymn" ||
                    existing.type === "liturgical_song"
                  ) {
                    const existingSongNumber = extractSongNumber(
                      existing.content,
                    );
                    ...
                    if (
                      existingSongNumber &&
                      newSongNumber &&
                      existingSongNumber === newSongNumber
                    ) {
                      score += 40;
                    }
                  }
      ...
      {showUserSelectModal && (
        <UserSelectionModal
          ...
          onSelect={(userName) =>
            debouncedAssignUser(showUserSelectModal.date, userName)
          }
          onDelete={() => handleRemoveReservation(showUserSelectModal.date)}
          title={
            showUserSelectModal.currentAssignment
              ? "Reassign Service"
              : "Assign User"
          }
        />
