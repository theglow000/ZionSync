...
const SeasonalTaggingTool = () => {
  ...
  const handleApplySeasons = async () => {
    if (selectedSeasons.length === 0 || Object.keys(selectedSongs).length === 0) {
      setUpdateStatus({
        type: 'error',
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow">
      ...
      {/* Season selection */}
      <div className="mb-4">
        <h3 className="text-sm font-medium text-gray-700 mb-2">Select Seasons to Apply/Remove:</h3>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button 
          ...
          disabled={isLoading || selectedSeasons.length === 0 || Object.keys(selectedSongs).length === 0}
        ...
        <button 
          ...
          disabled={isLoading || selectedSeasons.length === 0 || Object.keys(selectedSongs).length === 0}
          className={`px-3 py-2 rounded-md text-sm flex items-center ${
            isLoading ? 'bg-gray-300 text-gray-500' : 'bg-red-600 text-white hover:bg-red-700'
          }`}
        >
        ...
        <button 
          ...
          className={`px-3 py-2 rounded-md text-sm flex items-center ${
            isLoading ? 'bg-gray-300 text-gray-500' : 'bg-blue-600 text-white hover:bg-blue-700'
          }`}
        >
          {Object.keys(selectedSongs).length === filteredSongs.length && filteredSongs.length > 0 ? (
            <>
              <XIcon className="mr-1.5 w-4 h-4" />
              Deselect All
            </>
      ...
      <div className="mt-4 text-sm text-gray-600">
        Selected {Object.keys(selectedSongs).length} songs and {selectedSeasons.length} seasons
      </div>
    </div>
  );
};
