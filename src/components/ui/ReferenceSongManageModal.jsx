...
const ReferenceSongManageModal = ({ 
  ...
  const handleSaveSong = async () => {
    ...
    if (!formData.seasonalTags.length) {
      setMessage({ type: 'error', text: 'At least one season must be selected' });
      return;
    }
    ...
    if (formData.type === 'contemporary' && !formData.author) {
      setMessage({ type: 'error', text: 'Author/artist is required for contemporary songs' });
      return;
    }
    ...
    try {
      ...
      setMessage({ 
        type: 'success', 
        text: isUpdate ? 'Song updated successfully' : 'Song created successfully'
      });
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-gray-900 truncate flex items-center">
                            ...
                            {song.type === 'hymn' && song.number && (
                              <span className="ml-1 text-blue-700">#{song.number}</span>
                            )}
                          </div>
                          <div className="text-xs text-gray-500 flex items-center">
                            <span className={`inline-block w-2 h-2 rounded-full mr-1 ${
                              song.type === 'hymn' ? 'bg-blue-500' : 'bg-green-500'
                            }`}></span>
                      ...
                      {expandedSongId === song._id && (
                        <div className="p-3 bg-gray-50 border-t text-sm">
                          {song.seasonalTags && song.seasonalTags.length > 0 && (
                            <div className="mb-2">
                              <div className="font-medium text-xs text-gray-500 uppercase mb-1">Seasons</div>
                              <div className="flex flex-wrap gap-1">
                                {song.seasonalTags.map(tagId => {
                                  const season = liturgicalSeasons.find(s => s.id === tagId);
                                  return season && (
                                    <span 
                                      key={tagId}
                                      className="px-2 py-0.5 text-xs rounded-md border border-gray-200"
                                      style={{ borderLeftColor: season.color, borderLeftWidth: '3px' }}
                                    >
                                      {season.name}
                                    </span>
                                  );
                          ...
                          {(song.tempo || song.theme || song.arrangement) && (
                            <div className="mt-2 text-gray-700 text-sm">
                              {song.tempo && <div><span className="font-medium">Tempo:</span> {song.tempo}</div>}
                              {song.theme && <div><span className="font-medium">Theme:</span> {song.theme}</div>}
                              {song.arrangement && <div><span className="font-medium">Arrangement:</span> {song.arrangement}</div>}
                            </div>
                          )}
            ...
            <div className="flex-grow overflow-y-auto p-4">
              ...
              {confirmDelete && (
                <div className="mb-4 p-3 border border-red-300 bg-red-50 rounded-md">
                  <p className="text-red-700 mb-2 font-medium">Are you sure you want to delete this song?</p>
                  <p className="text-red-600 mb-3 text-sm">This action cannot be undone.</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex border rounded overflow-hidden">
                      <button
                        ...
                        onClick={() => setFormData(prev => ({ ...prev, type: 'hymn' }))}
                      >
                        Hymn
                      </button>
                      <button
                        ...
                        onClick={() => setFormData(prev => ({ ...prev, type: 'contemporary' }))}
                      >
                        Contemporary
                      </button>
                    </div>
                ...
                {/* Seasonal tags */}
                <div>
                  <h4 className="font-medium mb-2 text-gray-700">Seasonal Appropriateness*</h4>
                  <p className="text-sm text-gray-500 mb-2">Select all appropriate seasons</p>
              ...
              {/* New simplified fields */}
              <div className="mt-4">
                <h4 className="font-medium mb-2 text-gray-700">Song Characteristics</h4>
