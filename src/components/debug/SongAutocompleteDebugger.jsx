...
const SongAutocompleteDebugger = () => {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        ...
        console.log(`📈 Breakdown: ${hymns.length} hymns, ${contemporary.length} contemporary`);
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'monospace' }}>
      ...
      <p><strong>Total Songs:</strong> {availableSongs.hymn.length + availableSongs.contemporary.length}</p>
      <p><strong>Hymns:</strong> {availableSongs.hymn.length}</p>
      <p><strong>Contemporary:</strong> {availableSongs.contemporary.length}</p>
      
      <h3>Sample Hymns:</h3>
      <ul>
        {availableSongs.hymn.slice(0, 5).map(song => (
          <li key={song._id}>{song.title} #{song.number} ({song.hymnal})</li>
        ))}
      </ul>
      ...
      <ul>
        {availableSongs.contemporary.slice(0, 5).map(song => (
          <li key={song._id}>{song.title} - {song.author}</li>
        ))}
      </ul>
    </div>
  );
