...
const EditServiceModal = ({ isOpen, onClose, service, year, onSuccess }) => {
  ...
  useEffect(() => {
    if (service) {
      const isCustomColor = !LITURGICAL_COLORS.some(
        (c) => c.value === service.seasonColor,
      );
  ...
  const handleRevert = async () => {
    ...
    if (
      !confirm(
        "Are you sure you want to revert this service to its original state? This will remove all manual overrides for this service only.",
      )
    ) {
      return;
    }
  ...
  const isOverridden =
    service.isOverridden || formData.seasonName !== service.seasonName;
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700">
                    This service has been manually modified. Changes may cause
                    inconsistencies with the liturgical calendar.
                  </p>
          ...
          <div className="space-y-4">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      {showCustomColor
                        ? "Custom Color"
                        : LITURGICAL_COLORS.find(
                            (c) => c.value === formData.seasonColor,
                          )?.name || "Select a color"}
                    </span>
                  </div>
                  <svg
              ...
              {showCustomColor && (
                <div className="flex items-center gap-3 mt-3">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, seasonColor: e.target.value })
                    }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, specialDayName: e.target.value })
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900"
