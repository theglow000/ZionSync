...
ServiceCalendarSchema.methods.getServicesByDateRange = function (
  startDate,
  endDate,
) {
...
ServiceCalendarSchema.pre("save", function (next) {
  if (this.services && this.services.length > 0) {
    ...
    this.metadata.regularSundays = this.services.filter(
      (s) => s.isRegularSunday,
    ).length;
    this.metadata.specialWeekdays = this.services.filter(
      (s) => s.isSpecialWeekday,
    ).length;
    this.metadata.overriddenCount = this.services.filter(
      (s) => s.isOverridden,
    ).length;
  }
  next();
...
const ServiceCalendar =
  mongoose.models.ServiceCalendar ||
  mongoose.model("ServiceCalendar", ServiceCalendarSchema);
