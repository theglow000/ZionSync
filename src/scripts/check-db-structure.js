...
async function checkDatabaseStructure() {
  try {
    ...
    if (collections.find(c => c.name === "service_details")) {
      const serviceCount = await db.collection("service_details").countDocuments();
      console.log("service_details collection contains", serviceCount, "documents");
      
      // Get a sample service document to see its structure
      if (serviceCount > 0) {
        const sampleService = await db.collection("service_details").findOne({});
        console.log("Sample service_details document:", JSON.stringify(sampleService, null, 2));
      }
    }
    
    // Check other potential service collections
    if (collections.find(c => c.name === "service_songs")) {
      const serviceCount = await db.collection("service_songs").countDocuments();
      console.log("service_songs collection contains", serviceCount, "documents");
      
      if (serviceCount > 0) {
        const sampleService = await db.collection("service_songs").findOne({});
        console.log("Sample service_songs document:", JSON.stringify(sampleService, null, 2));
      }
    }
    ...
    for (const collection of collections) {
      if (collection.name.includes("service") && 
        ...
        console.log(`${collection.name} collection contains ${count} documents`);
        
        if (count > 0) {
          const sample = await db.collection(collection.name).findOne({});
          console.log(`Sample ${collection.name} document:`, JSON.stringify(sample, null, 2));
        }
      }
    }
    ...
    for (const collection of collections) {
      if (collection.name.includes("service")) {
        ...
        if (withDates.length > 0) {
          console.log("Date formats found:", withDates.map(d => d.date).slice(0, 5));
          ...
          console.log(`Found ${upcomingByString.length} potentially upcoming services`);
          
          if (upcomingByString.length > 0) {
            console.log("First 3 upcoming services:", upcomingByString.slice(0, 3).map(s => s.date));
          }
        }
      }
    }
