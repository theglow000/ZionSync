...
} from "../lib/liturgical-validation.js";
...
console.log(
  `🔄 Liturgical Season Migration ${DRY_RUN ? "(DRY RUN MODE)" : "(LIVE MODE)"}`,
);
  ...
  try {
    ...
    for (const service of serviceDetails) {
      try {
        ...
        const [month, day, yearShort] = service.date
          .split("/")
        ...
        try {
          ...
          console.error(
            `⚠️  Skipping service with invalid date: ${service.date} - ${error.message}`,
          );
          continue;
        }
        ...
        try {
          ...
          console.error(
            `⚠️  Invalid liturgical data for ${service.date}:`,
            error.message,
          );
          continue;
        }
        ...
        if (DRY_RUN) {
          console.log(
            `[DRY RUN] Would update service ${service.date}: ${liturgical.seasonName} (${liturgical.color})`,
          );
        } else {
          await db
            .collection("serviceDetails")
          ...
          console.log(
            `✅ Updated service ${service.date}: ${liturgical.seasonName} (${liturgical.color})`,
          );
        }
        ...
        if (updatedDetailsCount % 5 === 0) {
          console.log(
            `${DRY_RUN ? "Processed" : "Updated"} ${updatedDetailsCount}/${serviceDetails.length} service details...`,
          );
        }
      } catch (error) {
    ...
    console.log(
      `Updated liturgical info for ${updatedDetailsCount} services in serviceDetails collection.`,
    );
    ...
    for (const service of serviceSongs) {
      try {
        ...
        console.log(
          `Updated song selection ${service.date}: ${liturgical.seasonName}`,
        );

        updatedSongsCount++;
        if (updatedSongsCount % 5 === 0) {
          console.log(
            `Updated ${updatedSongsCount}/${serviceSongs.length} service songs...`,
          );
        }
      } catch (error) {
        console.error(`Error updating service ${service.date}:`, error);
    ...
    console.log(
      `Updated liturgical info for ${updatedSongsCount} services in service_songs collection.`,
    );
