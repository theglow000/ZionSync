...
years.forEach((year) => {
  ...
  try {
    ...
    console.log(
      `   - Validation: ${yearData.validated ? "✅ PASSED" : "❌ FAILED"}`,
    );

    // Display key liturgical dates
    console.log("\n🗓️  KEY DATES:");
    console.log(
      `   Ash Wednesday:    ${yearData.keyDates.ashWednesday?.toDateString() || "N/A"}`,
    );
    console.log(
      `   Palm Sunday:      ${yearData.keyDates.palmSunday?.toDateString() || "N/A"}`,
    );
    console.log(
      `   Maundy Thursday:  ${yearData.keyDates.maundyThursday?.toDateString() || "N/A"}`,
    );
    console.log(
      `   Good Friday:      ${yearData.keyDates.goodFriday?.toDateString() || "N/A"}`,
    );
    console.log(
      `   Easter Sunday:    ${yearData.keyDates.easter?.toDateString() || "N/A"}`,
    );
    console.log(
      `   Ascension:        ${yearData.keyDates.ascension?.toDateString() || "N/A"}`,
    );
    console.log(
      `   Pentecost:        ${yearData.keyDates.pentecost?.toDateString() || "N/A"}`,
    );
    console.log(
      `   Advent Starts:    ${yearData.keyDates.adventStart?.toDateString() || "N/A"}`,
    );
    console.log(
      `   Christmas Eve:    ${yearData.keyDates.christmasEve?.toDateString() || "N/A"}`,
    );
    ...
    specialWeekdays.forEach((service, index) => {
      ...
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${dateStr.padEnd(20)} ${service.specialDayName || service.specialDay}`,
      );
    });

    // Count Lenten midweek services
    const lentenMidweek = specialWeekdays.filter(
      (s) => s.specialDay === "LENT_MIDWEEK",
    );
    ...
    console.log("\n✅ DAY VERIFICATION:");
    console.log(
      `   Ash Wednesday is Wednesday:   ${ashWed?.getDay() === 3 ? "✅" : "❌"}`,
    );
    console.log(
      `   Maundy Thursday is Thursday:  ${maundyThu?.getDay() === 4 ? "✅" : "❌"}`,
    );
    console.log(
      `   Good Friday is Friday:        ${goodFri?.getDay() === 5 ? "✅" : "❌"}`,
    );

    // Verify Easter is Sunday
    const easter = yearData.keyDates.easter;
    console.log(
      `   Easter is Sunday:             ${easter?.getDay() === 0 ? "✅" : "❌"}`,
    );
    ...
    if (ashWed && easter) {
      ...
      console.log(
        `   Ash Wed to Easter (46 days):  ${daysBetween === 46 ? "✅" : `❌ (${daysBetween})`}`,
      );
    }
    ...
    if (lentenMidweek.length > 0) {
      ...
      const allBetween = lentenMidweek.every(
        (s) => s.date > ashWed && s.date < palmSunday && s.date.getDay() === 3,
      );
      console.log(
        `   Lenten services are Wednesdays: ${allBetween ? "✅" : "❌"}`,
      );
    }
