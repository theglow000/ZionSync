...
async function checkSongStructure() {
  try {
    ...
    const songCollections = collections.filter(c => 
      c.name === "songs" || 
      c.name === "reference_songs" || 
      c.name.includes("song")
    );
    
    console.log("Song-related collections:", songCollections.map(c => c.name));
    ...
    if (collections.find(c => c.name === "songs")) {
      ...
      if (songCount > 0) {
        ...
        console.log("Sample song document:", JSON.stringify(sampleSong, null, 2));
      }
    }
    ...
    if (collections.find(c => c.name === "reference_songs")) {
      ...
      const refSongCount = await db.collection("reference_songs").countDocuments();
      console.log("reference_songs collection contains", refSongCount, "documents");
      
      if (refSongCount > 0) {
        const sampleRefSong = await db.collection("reference_songs").findOne({});
        console.log("Sample reference song document:", JSON.stringify(sampleRefSong, null, 2));
      }
    }
    
    // 4. Look at how songs are structured in services
    console.log("\n=== SONGS IN SERVICES ===");
    const serviceWithSongs = await db.collection("serviceDetails").findOne(
      { "elements.type": "song_hymn" }
    );
    ...
    if (serviceWithSongs) {
      ...
      const songElements = serviceWithSongs.elements.filter(e => e.type === "song_hymn");
      console.log(`Service has ${songElements.length} song elements`);
      
      if (songElements.length > 0) {
        console.log("\nSample song element in service:", JSON.stringify(songElements[0], null, 2));
      }
    } else {
    ...
    if (collections.find(c => c.name === "song_usage")) {
      ...
      if (usageCount > 0) {
        ...
        console.log("Sample song usage record:", JSON.stringify(sampleUsage, null, 2));
      }
    }
    ...
    if (collections.find(c => c.name === "service_songs")) {
      const serviceSongCount = await db.collection("service_songs").countDocuments();
      console.log("service_songs collection contains", serviceSongCount, "documents");
      
      if (serviceSongCount > 0) {
        const sampleServiceSong = await db.collection("service_songs").findOne({});
        console.log("Sample service_songs document:", JSON.stringify(sampleServiceSong, null, 2));
      }
    }
