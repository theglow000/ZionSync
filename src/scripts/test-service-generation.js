...
try {
  ...
  if (!yearData.validated) {
    console.error("\n❌ Validation failed:");
    yearData.validationErrors.forEach((error) =>
      console.error(`   - ${error}`),
    );
    process.exit(1);
  }
  ...
  if (yearData.validationWarnings && yearData.validationWarnings.length > 0) {
    ...
    yearData.validationWarnings.forEach((warning) =>
  ...
  yearData.services.slice(0, 10).forEach((service, index) => {
    ...
    console.log(
      `${(index + 1).toString().padStart(2)}. ${dateStr.padEnd(8)} ${dayOfWeek} ${seasonName} ${specialDay}`,
    );
  });
  ...
  specialWeekdays.forEach((service, index) => {
    ...
    console.log(
      `${(index + 1).toString().padStart(2)}. ${dateStr.padEnd(8)} ${dayOfWeek} ${specialDay}`,
    );
  });
  ...
  console.log(
    `Match:       ${easter.toDateString() === "Sun Apr 20 2025" ? "✅ CORRECT" : "❌ WRONG"}`,
  );
  ...
  console.log(
    `Match:       ${palmSunday.toDateString() === "Sun Apr 13 2025" ? "✅ CORRECT" : "❌ WRONG"}`,
  );
  ...
  console.log(
    `Match:       ${goodFriday.toDateString() === "Fri Apr 18 2025" ? "✅ CORRECT" : "❌ WRONG"}`,
  );
