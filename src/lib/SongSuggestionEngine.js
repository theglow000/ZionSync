...
class SongSuggestionEngine {
  ...
  async getSuggestions({
    ...
    try {
      ...
      if (!shouldRefresh && this.cachedSuggestions && this.cachedSuggestions.length > 0) {
        return this.filterCachedSuggestions(this.cachedSuggestions, { type, limit });
      }
      ...
      if (!currentSeason) {
        ...
        if (liturgicalInfo.daysToNextSeason && 
   ...
   */
  scoreSongs(songs, { usageLookup, currentSeason, upcomingSeason, daysToNextSeason, unusedMonths }) {
    const scores = {};
    const now = new Date();
    ...
    songs.forEach(song => {
      ...
      if (!usage || usage.count === 0) {
        ...
        const monthsAgo = (now.getFullYear() - lastUsed.getFullYear()) * 12 + 
        ...
        if (isSeasonal) {
          ...
          const isCurrentSeasonSong = currentSeason && song.seasonalTags.some(tag => {
            const seasonName = typeof currentSeason === 'string' ? currentSeason : 
          ...
          } else if (upcomingSeason && song.seasonalTags.some(tag => {
            const seasonName = typeof upcomingSeason === 'string' ? upcomingSeason :
                              (upcomingSeason.name || upcomingSeason.seasonName || '');
            return tag.toLowerCase() === seasonName.toLowerCase();
          })) {
            // Upcoming season song - priority based on how close the season is
            const urgencyBoost = Math.min(50, (UPCOMING_SEASON_THRESHOLD_DAYS - daysToNextSeason) * 1.5);
      ...
      if (song.seasonalTags && song.seasonalTags.length > 0) {
        ...
        if (currentSeason && song.seasonalTags.some(tag => {
          const seasonName = typeof currentSeason === 'string' ? currentSeason : 
                            (currentSeason.name || currentSeason.seasonName || '');
          return tag.toLowerCase() === seasonName.toLowerCase();
        })) {
        ...
        if (upcomingSeason && song.seasonalTags.some(tag => {
          const seasonName = typeof upcomingSeason === 'string' ? upcomingSeason :
                            (upcomingSeason.name || upcomingSeason.seasonName || '');
          return tag.toLowerCase() === seasonName.toLowerCase();
        })) {
          const urgencyBoost = Math.min(45, (UPCOMING_SEASON_THRESHOLD_DAYS - daysToNextSeason) * 1.5);
          score += urgencyBoost;
   ...
   */
  getLiturgicalContext(song, { usageLookup, currentSeason, upcomingSeason, daysToNextSeason }) {
    const context = {
      isSeasonSong: false,
      seasonMatches: [],
      upcomingSeason: null,
    ...
    if (usage) {
      ...
      if (usage.lastUsed) {
        const lastUsed = new Date(usage.lastUsed);
        const now = new Date();
        context.monthsSinceLastUsed = (now.getFullYear() - lastUsed.getFullYear()) * 12 + 
                                      (now.getMonth() - lastUsed.getMonth());
      }
    }
    ...
    if (song.seasonalTags && song.seasonalTags.length > 0) {
      ...
      song.seasonalTags.forEach(tag => {
        if (!context.seasonMatches.some(match => match.season.toLowerCase() === tag.toLowerCase())) {
          context.seasonMatches.push({
            season: tag,
            isCurrent: false,
  ...
  getSeasonName(seasonId) {
    ...
    const season = Object.entries(LITURGICAL_SEASONS).find(([key]) => 
  ...
  applyVarietyAlgorithm(sortedSongs) {/* unchanged */}
  balanceSongTypes(songs, limit) {/* unchanged */}
  recordSuggestions(songs, season) {/* unchanged */}
  filterCachedSuggestions(suggestions, { type, limit }) {/* unchanged */}
  resetSuggestions() {/* unchanged */}
}
