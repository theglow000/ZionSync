...
describe('ServiceGenerator', () => {
  describe('generateServicesForYear', () => {
    ...
    test('includes Christmas Eve as special weekday (2025 - Wednesday)', () => {
      ...
      const christmasEve = result.services.find(s => s.specialDay === 'CHRISTMAS_EVE');
  ...
  describe('Liturgical Season Coverage', () => {
    ...
    test('includes Palm Sunday', () => {
      ...
      const palmSunday = result.services.find(s => s.specialDay === 'PALM_SUNDAY');
    ...
    test('includes Pentecost Sunday', () => {
      ...
      const pentecost = result.services.find(s => s.specialDay === 'PENTECOST_SUNDAY');
    ...
    test('includes Trinity Sunday', () => {
      ...
      const trinity = result.services.find(s => s.specialDay === 'TRINITY_SUNDAY');
    ...
    test('includes All Saints Day (LCMC tradition)', () => {
      ...
      const allSaints = result.services.find(s => s.specialDay === 'ALL_SAINTS_DAY');
    ...
    test('includes Reformation Sunday', () => {
      ...
      const reformation = result.services.find(s => s.specialDay === 'REFORMATION_SUNDAY');
    ...
    test('includes Christ the King Sunday', () => {
      ...
      const christTheKing = result.services.find(s => s.specialDay === 'CHRIST_THE_KING');
  ...
  describe('Special Weekday Services', () => {
    test('Ash Wednesday is 46 days before Easter', () => {
      ...
      const ashWednesday = result.services.find(s => s.specialDay === 'ASH_WEDNESDAY');
      const easter = result.keyDates.easter;
      
      expect(ashWednesday).toBeDefined();
      
      const daysBetween = Math.round((easter - ashWednesday.date) / (1000 * 60 * 60 * 24));
    ...
    test('Maundy Thursday is 3 days before Easter', () => {
      ...
      const maundyThursday = result.services.find(s => s.specialDay === 'MAUNDY_THURSDAY');
      const easter = result.keyDates.easter;
      
      expect(maundyThursday).toBeDefined();
      
      const daysBetween = Math.round((easter - maundyThursday.date) / (1000 * 60 * 60 * 24));
    ...
    test('Good Friday is 2 days before Easter', () => {
      ...
      const goodFriday = result.services.find(s => s.specialDay === 'GOOD_FRIDAY');
      const easter = result.keyDates.easter;
      
      expect(goodFriday).toBeDefined();
      
      const daysBetween = Math.round((easter - goodFriday.date) / (1000 * 60 * 60 * 24));
    ...
    test('Ascension Day is 39 days after Easter', () => {
      ...
      const ascension = result.services.find(s => s.specialDay === 'ASCENSION');
      const easter = result.keyDates.easter;
      
      expect(ascension).toBeDefined();
      
      const daysBetween = Math.round((ascension.date - easter) / (1000 * 60 * 60 * 24));
  ...
  describe('Date Formatting', () => {
    test('dateString matches MM/DD/YY format', () => {
      ...
      const easter = result.services.find(s => s.specialDay === 'EASTER_SUNDAY');
    ...
    test('handles single-digit months and days', () => {
      ...
      const januaryService = result.services.find(s => 
        s.date.getMonth() === 0 && s.date.getDate() === 5
      );
  ...
  describe('Validation', () => {
    ...
    test('detects if Easter is missing (hypothetical)', () => {
      ...
      const hasEaster = result.services.some(s => s.specialDay === 'EASTER_SUNDAY');
    ...
    test('warns about large gaps (should not happen)', () => {
      ...
      const gapWarnings = result.validationWarnings?.filter(w => w.includes('Gap of')) || [];
