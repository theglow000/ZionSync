...
export function generateServicesForYear(year) {
  ...
  if (!easterValidation.isValid) {
    throw new Error(`Easter validation failed for ${year}: ${easterValidation.message}`);
  }
...
function generateSpecialWeekdays(year) {
  ...
  if (christmasEve.getDay() !== 0) {  // Only add if not Sunday
    specialServices.push(createSpecialWeekdayService(christmasEve, 'CHRISTMAS_EVE'));
  }
  
  // Ash Wednesday - always observed (start of Lent)
  if (ashWednesday.getDay() !== 0) {  // Should never be Sunday, but check anyway
    specialServices.push(createSpecialWeekdayService(ashWednesday, 'ASH_WEDNESDAY'));
  }
  
  // Maundy Thursday (Easter - 3 days)
  const maundyThursday = new Date(easter);
  maundyThursday.setDate(easter.getDate() - 3);
  specialServices.push(createSpecialWeekdayService(maundyThursday, 'MAUNDY_THURSDAY'));
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (lentenWednesday < palmSunday) {
      specialServices.push(createSpecialWeekdayService(lentenWednesday, 'LENT_MIDWEEK'));
  ...
  specialServices.push(createSpecialWeekdayService(thanksgivingEve, 'THANKSGIVING_EVE'));
...
function validateGeneratedServices(services, year) {
  ...
  if (!dateRangeValidation.isValid) {
    if (dateRangeValidation.duplicates.length > 0) {
      errors.push(`Found ${dateRangeValidation.duplicates.length} duplicate dates`);
    }
    if (dateRangeValidation.gaps.length > 0) {
      dateRangeValidation.gaps.forEach(gap => {
        warnings.push(`Gap of ${gap.daysBetween} days between ${gap.after} and ${gap.before}`);
      });
    }
  }
  ...
  const hasChristmas = services.some(s => s.specialDay === 'CHRISTMAS_DAY' || s.specialDay === 'CHRISTMAS_EVE');
  if (!hasChristmas) {
