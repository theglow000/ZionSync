...
export class SongMatcher {
  ...
  static async findPotentialMatches(referenceOrQuery, options = {}) {
    ...
    try {
      ...
      const queryTitle = isQueryString
        ? referenceOrQuery
        : referenceOrQuery.title;
      ...
      if (
        ...
        query = {
          ...
          $or: [
            ...
            {
              title: {
                $regex: this.createFuzzyRegex(queryTitle),
                $options: "i",
              },
            },
          ],
      ...
      const scoredMatches = matches.map((song) => {
        const score = this.calculateSimilarityScore(
          song,
          isQueryString ? { title: queryTitle } : referenceOrQuery,
        );
  ...
  static calculateSimilarityScore(songA, songB) {
    ...
    if (
      songA.type === "contemporary" &&
      songB.type === "contemporary" &&
      songA.author &&
      songB.author &&
