...
const baseSongSchema = z.object({
  ...
  type: z.enum(["hymn", "contemporary"], {
    errorMap: () => ({
      message: "Type must be either 'hymn' or 'contemporary'",
    }),
  }),
  notes: z
    .string()
...
const hymnSpecificSchema = z.object({
  number: z
    .string()
    .max(10, "Hymn number must be less than 10 characters")
    .optional()
    .default(""),
  hymnal: z
    .string()
    .max(100, "Hymnal name must be less than 100 characters")
    .optional()
    .default(""),
...
export const songUsageSchema = z.object({
  ...
  date: z
    .string()
    .regex(/^\d{1,2}\/\d{1,2}\/\d{2}$/, "Date must be in MM/DD/YY format"),
  service: z
    .string()
  ...
  addedBy: z
    .string()
    .max(100, "Added by must be less than 100 characters")
    .optional()
    .default(""),
