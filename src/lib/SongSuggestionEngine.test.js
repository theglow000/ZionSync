...
describe('SongSuggestionEngine', () => {
  ...
  const mockSongs = [
    ...
    { _id: 'song3', title: 'Cornerstone', type: 'contemporary', seasonalTags: ['Advent', 'Christmas'] },
    { _id: 'song4', title: 'In Christ Alone', type: 'contemporary', seasonalTags: ['Easter', 'Pentecost'] }
  ];
  ...
  const mockUsageData = {
    ...
    frequency: [
      ...
      { title: 'How Great Thou Art', count: 3, lastUsed: new Date('2024-01-15') },
  ...
  describe('getSuggestions', () => {
    ...
    test('filters by song type', async () => {
      ...
      const allContemporary = result.every(song => song.type === 'contemporary');
  ...
  describe('seasonal filtering', () => {
    test('prioritizes songs matching current season', async () => {
      ...
      const easterSongs = result.filter(song => 
        song.seasonalTags && song.seasonalTags.includes('Easter')
      );
