...
describe("SongSuggestionEngine Performance", () => {
  ...
  const generateUsageData = (songs, usageFrequency) => {
    ...
    for (let i = 0; i < songsToUse; i++) {
      ...
      for (let j = 0; j < count; j++) {
        ...
        serviceDate.setDate(
          serviceDate.getDate() - Math.floor(Math.random() * 365),
        );
  ...
  beforeAll(() => {
    ...
    songSuggestionEngine.balanceSongTypes = jest.fn((songs, limit) =>
      songs.slice(0, limit),
    );
    songSuggestionEngine.getLastSeasonStart = jest.fn(
      () => new Date(2023, 0, 1),
    );
    songSuggestionEngine.getSeasonName = jest.fn(
  ...
  test.each([
    ...
    "handles different usage densities with %i songs and %f density",
    async (songCount, usageFrequency) => {
      // Generate test data
      const songs = generateSongs(songCount);
      const usageData = generateUsageData(songs, usageFrequency);
