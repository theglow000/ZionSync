    ...
    try {
        ...
        expectedFunctions.forEach(funcName => {
            ...
            addResult(
                ...
                exists ? 'PASS' : 'FAIL',
                exists ? null : `Function ${funcName} not exported from LiturgicalCalendarService`
            );
        });
        ...
        try {
            ...
            addResult(
                ...
                season ? `Returned: ${season}` : 'Function returned null or invalid value'
            );
            
            addResult(
                'LiturgicalCalendarService',
                'getSeasonColor returns valid color',
                seasonColor && typeof seasonColor === 'string' && seasonColor.startsWith('#') ? 'PASS' : 'FAIL',
                seasonColor ? `Returned: ${seasonColor}` : 'Function returned null or invalid color'
            );
            ...
            addResult(
                ...
                liturgicalInfo ? `Has seasonId: ${!!liturgicalInfo.seasonId}, season: ${!!liturgicalInfo.season}` : 'Function returned null or invalid object'
            );
...
async function verifyLiturgicalSeasons() {
    ...
    try {
        const { LITURGICAL_SEASONS, MAJOR_FEAST_DAYS } = await import('./src/lib/LiturgicalSeasons.js');
        ...
        Object.entries(expectedSeasons).forEach(([seasonId, expectedColor]) => {
            ...
            addResult(
                ...
                exists ? 'PASS' : 'FAIL',
                exists ? null : `Expected color ${expectedColor}, got ${seasonData?.color || 'undefined'}`
            );
        });
        ...
        addResult(
            ...
            MAJOR_FEAST_DAYS && typeof MAJOR_FEAST_DAYS === 'object' ? 'PASS' : 'FAIL',
            MAJOR_FEAST_DAYS ? `Contains ${Object.keys(MAJOR_FEAST_DAYS).length} feast days` : 'MAJOR_FEAST_DAYS not found'
        );
...
async function verifyLiturgicalCSS() {
    ...
    try {
        ...
        const expectedSeasons = ['advent', 'christmas', 'lent', 'easter', 'epiphany'];
...
async function verifyLiturgicalComponents() {
    ...
    if (stylingExists) {
        try {
            ...
            addResult(
                ...
                hasGetSeasonClass ? null : 'getSeasonClass function not found in component'
            );
            ...
            addResult(
                ...
                hasGetHeaderClass ? null : 'getHeaderClass function not found in component'
            );
    ...
    if (planningExists) {
        try {
            ...
            const hasSeasonalPlanning = planningContent.includes('SeasonalPlanningGuide');
            const usesCurrentSeason = planningContent.includes('getCurrentSeason') || 
            ...
            addResult(
                ...
                hasSeasonalPlanning ? null : 'SeasonalPlanningGuide component not found'
            );
            ...
            addResult(
                ...
                usesCurrentSeason ? null : 'No season detection usage found in component'
            );
...
async function verifyAPIIntegration() {
    ...
    if (upcomingServicesExists) {
        try {
            ...
            const usesLiturgicalService = upcomingContent.includes('getLiturgicalInfo') ||
            ...
            addResult(
                ...
                usesLiturgicalService ? null : 'No liturgical service integration found in upcoming-services API'
            );
    ...
    if (serviceSongsExists) {
        try {
            ...
            const usesLiturgicalService = serviceSongsContent.includes('getLiturgicalInfo') ||
            ...
            addResult(
                ...
                usesLiturgicalService ? 'PASS' : 'FAIL',
                usesLiturgicalService ? null : 'No liturgical context found in service-songs API'
            );
...
async function verifyMigrationScripts() {
    ...
    for (const scriptPath of expectedScripts) {
        const exists = await verifyFile(scriptPath, `Migration script ${path.basename(scriptPath)} exists`);
        
        if (exists) {
            try {
                const scriptContent = fs.readFileSync(path.join(__dirname, scriptPath), 'utf8');
                
                const usesLiturgicalService = scriptContent.includes('getLiturgicalInfo') ||
                ...
                addResult(
                    ...
                    usesLiturgicalService ? 'PASS' : 'FAIL',
                    usesLiturgicalService ? null : 'Script does not import or use liturgical service'
                );
...
async function verifyCalendarDataSource() {
    ...
    if (calendarExists) {
        try {
            ...
            const hasSeasonStructure = calendarContent.includes('Advent') && 
            ...
            addResult(
                ...
                hasSeasonStructure ? null : 'Calendar does not contain expected liturgical season structure'
            );
...
async function main() {
    ...
    const accuracy = results.totalChecks > 0 ? (results.passed / results.totalChecks * 100).toFixed(1) : 0;
    ...
    console.log('💾 Results saved to liturgical-integration-verification-results.json');
