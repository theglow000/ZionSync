...
async function testPastorSongReduction() {
  ...
  try {
    ...
    console.log("==========================================\n");
    console.log(
      "SCENARIO: Pastor creates 5 songs → Worship team selects all 5 → Pastor reduces to 3",
    );
    ...
    const addSongsResponse = await fetch(
      "http://localhost:3000/api/service-songs",
      {
        method: "POST",
    ...
    const afterSongSelection = await db
      .collection("serviceDetails")
      .findOne({ date: testDate });
    const songsAfterSelection = afterSongSelection.elements.filter(
      (e) => e.selection?.title,
    );
    ...
    const afterReduction = await db
      .collection("serviceDetails")
      .findOne({ date: testDate });
    const songsAfterReduction = afterReduction.elements.filter(
      (e) => e.type === "song_hymn",
    );
    const songsWithSelections = songsAfterReduction.filter(
      (e) => e.selection?.title,
    );
    ...
    console.log(
      `📉 Reduced service: ${songsAfterReduction.length} song slots, ${songsWithSelections.length} selections`,
    );
    ...
    songsAfterReduction.forEach((song, index) => {
      if (song.selection?.title) {
        console.log(
          `   ✅ Position ${index + 1}: ${song.content} → ${song.selection.title}`,
        );
      } else {
        console.log(
          `   📝 Position ${index + 1}: ${song.content} → PENDING (no selection)`,
        );
      }
    });
    ...
    console.log(
      `1. Were orphaned songs removed? ${question1 ? "✅ YES" : "❌ NO"}`,
    );
    console.log(
      `   → Result: ${songsWithSelections.length} songs preserved from original 5`,
    );

    const originalSongTitles = songsAfterSelection.map(
      (s) => s.selection.title,
    );
    const currentSongTitles = songsWithSelections.map((s) => s.selection.title);
    const preservedSongs = currentSongTitles.filter((title) =>
      originalSongTitles.includes(title),
    );
    ...
    const lostSongs = originalSongTitles.filter(
      (title) => !currentSongTitles.includes(title),
    );
    ...
    console.log(
      `4. Does the service structure match pastor's intent? ${question4 ? "✅ YES" : "❌ NO"}`,
    );
    console.log(
      `   → Service now has ${songsAfterReduction.length} song slots as intended`,
    );
    ...
    const testNewSong = {
      ...
      song_1: {
        title: "NEW SONG: Crown Him with Many Crowns",
        type: "hymn",
        number: "855",
        hymnal: "cranberry",
      },
    ...
    console.log(
      `   ${systemStillWorks ? "✅" : "❌"} Can still add songs to remaining slots`,
    );
    ...
    console.log(
      `   📊 Final working state: ${finalSongs.length}/3 songs filled`,
    );
    ...
    if (lostSongs.length > 0) {
      console.log(
        `❌ ORPHANED SONGS ARE LOST: ${lostSongs.length} songs disappeared`,
      );
      console.log(
        "   - This is problematic behavior that needs to be addressed",
      );
      console.log("   - Lost songs:", lostSongs.join(", "));
    } else {
      console.log(
        "✅ NO ORPHANED SONGS: All songs were preserved or gracefully handled",
      );
    }
