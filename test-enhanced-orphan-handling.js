...
async function testEnhancedOrphanHandling() {
  ...
  try {
    ...
    const allFiveSongs = {
      song_0: { title: "Holy, Holy, Holy", type: "hymn", number: "413", hymnal: "cranberry" },
      song_1: { title: "Gather Us In", type: "hymn", number: "532", hymnal: "cranberry" },
      song_2: { title: "Be Thou My Vision", type: "hymn", number: "793", hymnal: "cranberry" },
      song_3: { title: "Let Us Break Bread Together", type: "hymn", number: "471", hymnal: "cranberry" },
      song_4: { title: "Go in Peace", type: "hymn", number: "634", hymnal: "cranberry" }
    };
    ...
    const reducedService = {
      ...
      lastUpdated: (await db.collection("serviceDetails").findOne({ date: testDate })).lastUpdated
    };
    ...
    console.log('✅ Pastor edit completed - enhanced handling should have triggered');
    
    // Step 4: Verify orphaned songs were backed up
    console.log('\n🔍 Step 4: Verifying orphaned song backup system...');
    
    const orphanedRecords = await db.collection("orphaned_songs").find({ date: testDate }).toArray();
    ...
    if (serviceSongs) {
      const validSongs = Object.entries(serviceSongs.selections).filter(([k,v]) => v?.title).length;
      console.log(`📊 Songs in service_songs collection: ${validSongs}`);
      console.log(`🔄 Last synced: ${serviceSongs.lastSyncedWithServiceDetails || 'Never'}`);
      console.log(`❌ Orphaned songs removed: ${serviceSongs.orphanedSongsRemoved || 0}`);
    }
    ...
    const finalSongs = finalService.elements.filter(e => e.type === 'song_hymn' && e.selection?.title);
    
    console.log(`📊 Final service song count: ${finalSongs.length}/3`);
    console.log('🎵 Preserved songs:');
    finalSongs.forEach((song, index) => {
      console.log(`   ${index + 1}. ${song.selection.title} (${song.content.split(':')[0]})`);
    });
    ...
    const tests = [
      ...
      { name: 'Backup Records Created', passed: orphanedRecords.length > 0 && orphanedRecords[0].orphanedSongs.length === 2 },
      { name: 'Service Songs Synced', passed: serviceSongs && serviceSongs.orphanedSongsRemoved === 2 },
      { name: 'Songs Preserved Correctly', passed: finalSongs.length === 3 },
      { name: 'Orphan Tracking Added', passed: finalService.lastOrphanEvent?.orphanCount === 2 },
      { name: 'System Still Functional', passed: finalSongs.every(s => s.selection?.title) }
    ];
