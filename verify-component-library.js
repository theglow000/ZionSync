...
function analyzeComponentFile(filePath) {
  try {
    ...
    const namedExportMatches = content.matchAll(
      /export\s+(?:const\s+|function\s+|class\s+)?(\w+)/g,
    );
...
function verifyComponent(name, documented) {
  ...
  if (documented.exportType === "named" && documented.namedExports) {
    ...
    const missingExports = documented.namedExports.filter(
      (exp) => !actual.exports.includes(exp),
    );
    if (missingExports.length > 0) {
      console.log(
        `  ${colors.yellow}⚠️  Missing named exports: ${missingExports.join(", ")}${colors.reset}`,
      );
      results.discrepancies.push({
    ...
    if (!actual.hasDefaultExport) {
      console.log(
        `  ${colors.yellow}⚠️  Component should have default export${colors.reset}`,
      );
      results.discrepancies.push({
  ...
  if (interactiveComponents.includes(name) && !actual.isClientComponent) {
    console.log(
      `  ${colors.yellow}⚠️  Interactive component should have 'use client' directive${colors.reset}`,
    );
    results.discrepancies.push({
    ...
    console.log(
      `  ${colors.green}✅ Client component directive found${colors.reset}`,
    );
  }
...
function verifyHook(name, documented) {
  ...
  if (!actual.hasDefaultExport) {
    console.log(
      `  ${colors.yellow}⚠️  Hook should have default export${colors.reset}`,
    );
    results.discrepancies.push({
      type: "hook-export",
      hook: name,
...
function findUndocumentedComponents() {
  ...
  for (const filePath of allFiles) {
    ...
    const isDocumented = Object.values(documentedComponents).some(
      (comp) => comp.location === relativePath,
    );
    ...
    if (!isDocumented) {
      undocumented.push({
        name: fileName,
        path: relativePath,
        type: filePath.includes("ui/")
          ? "ui"
          : filePath.includes("liturgical/")
            ? "liturgical"
            : "layout",
      });
  ...
  if (undocumented.length > 0) {
    ...
    undocumented.forEach((comp) => {
      console.log(
        `  ${colors.yellow}• ${comp.name}${colors.reset} (${comp.path})`,
      );
      results.discrepancies.push({
    ...
    console.log(
      `${colors.green}✅ All components are documented${colors.reset}`,
    );
  }
}
...
function generateReport() {
  console.log(`\n${"=".repeat(80)}`);
  console.log(
    `${colors.bold}${colors.blue}COMPONENT LIBRARY VERIFICATION REPORT${colors.reset}`,
  );
  ...
  console.log(
    `  Components found: ${colors.green}${results.foundComponents}${colors.reset}`,
  );
  console.log(
    `  Components missing: ${colors.red}${results.missingComponents}${colors.reset}`,
  );
  console.log(
    `  Undocumented components: ${colors.yellow}${results.unexpectedComponents}${colors.reset}`,
  );
  console.log(
    `  Total discrepancies: ${colors.red}${results.discrepancies.length}${colors.reset}`,
  );

  // Calculate accuracy
  const accuracy = Math.round(
    ((results.foundComponents - results.discrepancies.length) /
      results.totalComponents) *
      100,
  );
  console.log(
    `  ${colors.bold}Documentation Accuracy: ${accuracy}%${colors.reset}`,
  );
  ...
  if (results.missingComponents > 0) {
    console.log(
      `  ${colors.red}• Update component-library.md to remove ${results.missingComponents} missing components${colors.reset}`,
    );
  }

  if (results.unexpectedComponents > 0) {
    console.log(
      `  ${colors.yellow}• Add documentation for ${results.unexpectedComponents} undocumented components${colors.reset}`,
    );
  }

  const clientDirectiveIssues = results.discrepancies.filter(
    (d) => d.type === "client-directive",
  ).length;
  if (clientDirectiveIssues > 0) {
    console.log(
      `  ${colors.yellow}• Add 'use client' directive to ${clientDirectiveIssues} interactive components${colors.reset}`,
    );
  }

  const exportIssues = results.discrepancies.filter(
    (d) => d.type === "export-mismatch",
  ).length;
  if (exportIssues > 0) {
    console.log(
      `  ${colors.yellow}• Fix export patterns in ${exportIssues} components${colors.reset}`,
    );
  }

  if (results.discrepancies.length === 0) {
    console.log(
      `  ${colors.green}✅ No improvements needed - documentation is accurate!${colors.reset}`,
    );
  }
...
function main() {
  console.log(
    `${colors.bold}${colors.blue}ZionSync Component Library Verification${colors.reset}`,
  );
  console.log(
    `${colors.blue}Verifying component-library.md against actual implementation...${colors.reset}\n`,
  );
