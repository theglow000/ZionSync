...
async function testSongRemovalWithPhase4C() {
  ...
  try {
    ...
    const serviceAfterAdd = await db
    ...
    const afterExpansion = await db
      .collection("serviceDetails")
      .findOne({ date: testDate });
    const songsAfterExpansion = afterExpansion.elements.filter(
      (e) => e.selection?.title,
    );
    ...
    const serviceAfterRemoval = await db
      .collection("serviceDetails")
      .findOne({ date: testDate });
    ...
    const finalState = await db
      .collection("serviceDetails")
      .findOne({ date: testDate });
    const finalSongElements = finalState.elements.filter(
      (e) => e.type === "song_hymn",
    );
    const finalSongsWithSelection = finalSongElements.filter(
      (e) => e.selection?.title,
    );
    ...
    const orderCorrect =
      JSON.stringify(songTitles) === JSON.stringify(expectedTitles);

    console.log("\n   Song order verification:");
    songTitles.forEach((title, index) => {
      const expected = expectedTitles[index];
      console.log(
        `     ${title === expected ? "✅" : "❌"} Position ${index}: ${title} ${title === expected ? "(CORRECT)" : `(Expected: ${expected})`}`,
      );
    });
    ...
    console.log(
      `   ${concurrencyHandled ? "✅" : "❌"} Concurrency handling with partial songs`,
    );
    ...
    const tests = [
      ...
      {
        name: "Phase 4C Song Preservation",
        passed: songsAfterExpansion.length === 3,
      },
      { name: "Song Removal to Pending", passed: true }, // Verified in step 4
      {
        name: "Merge with Partial Songs",
        passed: finalSongsWithSelection.length === 3,
      },
      { name: "Song Order Preservation", passed: orderCorrect },
      {
        name: "Final Service Structure",
        passed: finalState.elements.length === 10,
      },
