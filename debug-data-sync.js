...
async function debugDataSync() {
  ...
  try {
    ...
    for (const date of dates) {
      ...
      const serviceSongs = await db.collection("service_songs").findOne({ date });
      console.log(`🎵 service_songs collection:`, serviceSongs ? 'EXISTS' : 'NOT FOUND');
      if (serviceSongs) {
        ...
        if (serviceSongs.selections) {
          Object.entries(serviceSongs.selections).forEach(([key, song]) => {
            if (song?.title) {
              console.log(`   ${key}: ${song.title}${song.number ? ` #${song.number}` : ''}${song.hymnal ? ` (${song.hymnal})` : ''}`);
            }
          });
        }
      }
      ...
      const serviceDetails = await db.collection("serviceDetails").findOne({ date });
      console.log(`📋 serviceDetails collection:`, serviceDetails ? 'EXISTS' : 'NOT FOUND');
      if (serviceDetails) {
        ...
        console.log(`   Elements count: ${serviceDetails.elements?.length || 0}`);
          ...
          serviceDetails.elements.forEach((element, index) => {
            if (element.type === 'song_hymn' || element.type === 'song_contemporary') {
              ...
              if (element.selection) {
                console.log(`     Selection: ${element.selection.title}${element.selection.number ? ` #${element.selection.number}` : ''}${element.selection.hymnal ? ` (${element.selection.hymnal})` : ''}`);
              } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ /api/service-details: ${data.length} services returned`);
        ...
        console.log(`❌ /api/service-details: ${response.status} ${response.statusText}`);
      }
    } catch (error) {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/service-songs?date=11/9/25');
      if (response.ok) {
        const data = await response.json();
        console.log(`✅ /api/service-songs for 11/9/25:`, data ? 'HAS DATA' : 'NO DATA');
        if (data?.selections) {
          Object.entries(data.selections).forEach(([key, song]) => {
            if (song?.title) {
              console.log(`   ${key}: ${song.title}${song.number ? ` #${song.number}` : ''}`);
            }
          });
        }
      } else {
        console.log(`❌ /api/service-songs: ${response.status} ${response.statusText}`);
      }
    } catch (error) {
