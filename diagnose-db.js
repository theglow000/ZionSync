...
async function diagnoseDatabase(uri) {
  ...
  try {
    ...
    const serviceDetailsCount = await db.collection('serviceDetails').countDocuments();
    ...
    const serviceCalendarCount = await db.collection('serviceCalendar').countDocuments();
    ...
    // Check service_songs
    const serviceSongsCount = await db.collection('service_songs').countDocuments();
...
rl.question('Enter your MongoDB URI (from Vercel environment variables): ', (uri) => {
  if (!uri || uri.trim() === '') {
