...
// Configuration
const DOCUMENTATION_FILE = path.join(
  __dirname,
  "GuidingDocs/song-management-system.md",
);
const RESULTS_FILE = path.join(
  __dirname,
  "song-management-verification-results.json",
);
...
class SongManagementVerifier {
  ...
  async initialize() {
    console.log(
      "🔍 Initializing Song Management System Documentation Verification...\n",
    );
    ...
    try {
      ...
      console.log(
        `✅ Documentation loaded (${this.docContent.length} characters)`,
      );
  ...
  async verifyDatabaseCollections() {
    ...
    for (const collectionName of documentedCollections) {
      ...
      results.push(
        this.checkExists(`Collection "${collectionName}" exists`, exists),
      );
    }
    ...
    try {
      ...
      if (sampleSong) {
        ...
        for (const field of keyFields) {
          ...
          results.push(
            this.checkExists(`Songs collection has "${field}" field`, exists),
          );
        }
        ...
        const documentedFields = [
          ...
          "lastUpdated",
          "usageCount",
          "songOrder", // Added fields for analytics and Proclaim integration
          "seasonalTags",
        ...
        const extraFields = actualFields.filter(
          (field) => !documentedFields.includes(field),
        );

        if (extraFields.length > 0) {
          console.log(
            `   ⚠️  Undocumented fields found: ${extraFields.join(", ")}`,
          );
          this.results.criticalFindings.push(
            `Songs collection has undocumented fields: ${extraFields.join(", ")}`,
          );
        } else {
      ...
      if (sampleUsage) {
        ...
        if (hasNestedStructure) {
          ...
          results.push(
            this.checkClaim(
              "song_usage has nested structure as documented",
              true,
              true,
            ),
          );
          ...
          this.results.criticalFindings.push(
            "MAJOR DISCREPANCY: song_usage collection is FLAT, not nested as documented",
          );
        }
  ...
  async verifyAPIEndpoints() {
    ...
    const documentedEndpoints = [
      ...
      {
        path: "/api/song-usage/suggestions",
        file: "src/app/api/song-usage/suggestions/route.js",
      },
      {
        path: "/api/song-usage/seasonal-type-distribution",
        file: "src/app/api/song-usage/seasonal-type-distribution/route.js",
      },
      {
        path: "/api/song-usage/seasonal-gaps",
        file: "src/app/api/song-usage/seasonal-gaps/route.js",
      },
      {
        path: "/api/song-usage/new-songs",
        file: "src/app/api/song-usage/new-songs/route.js",
      },
      {
        path: "/api/song-usage/check",
        file: "src/app/api/song-usage/check/route.js",
      },
      {
        path: "/api/song-usage/congregation-comfort",
        file: "src/app/api/song-usage/congregation-comfort/route.js",
      },
      {
        path: "/api/reference-songs",
        file: "src/app/api/reference-songs/route.js",
      },
    ];
  ...
  generateReport() {
    this.results.accuracy =
      this.results.totalChecks > 0
        ? Math.round((this.results.passed / this.results.totalChecks) * 100)
        : 0;
    ...
    if (this.results.accuracy >= 90) {
      ...
      console.log(
        "⚠️  GOOD: Documentation mostly accurate, some discrepancies found",
      );
    } else if (this.results.accuracy >= 60) {
      console.log(
        "⚠️  FAIR: Significant documentation discrepancies identified",
      );
    } else {
