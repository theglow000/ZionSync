...
const ARCHITECTURE_DOC = path.join(
  REPO_ROOT,
  "GuidingDocs",
...
function verifyTechnologyStack() {
  ...
  uiDeps.forEach((dep) => {
    addResult(
      ...
      packageJson.dependencies[dep]
        ? `Found: ${packageJson.dependencies[dep]}`
        : "Not found",
    );
  });
...
function verifyFileStructure() {
  ...
  const actualApiEndpoints = getDirectoryContents(apiDir).filter((item) =>
    item.endsWith("/"),
  );
  ...
  expectedApiEndpoints.forEach((endpoint) => {
    addResult(
      ...
      actualApiEndpoints.includes(endpoint + "/")
        ? null
        : "Directory not found in API routes",
    );
  });
}
...
function verifyComponentArchitecture() {
  ...
  if (fileExists(mainLayoutPath)) {
    ...
    addResult(
      ...
      mainLayoutContent.includes("useResponsive")
        ? null
        : "useResponsive not imported/used",
    );
  ...
  addResult(
    ...
    fs.existsSync(uiComponentsPath)
      ? null
      : "src/components/ui directory not found",
  );
}
...
function verifyLiturgicalIntegration() {
  ...
  const calendarServicePath = path.join(
    REPO_ROOT,
    "src/lib/LiturgicalCalendarService.js",
  );
  ...
  const liturgicalSeasonsPath = path.join(
    REPO_ROOT,
    "src/lib/LiturgicalSeasons.js",
  );
  if (fileExists(liturgicalSeasonsPath)) {
    ...
    expectedSeasons.forEach((season) => {
      addResult(
        ...
        content.includes(season)
          ? null
          : `${season} not found in liturgical seasons`,
      );
    });
  }
  ...
  addResult(
    ...
    fs.existsSync(liturgicalApiPath)
      ? null
      : "Liturgical API directory not found",
  );
}
...
function verifyDatabaseArchitecture() {
  ...
  if (fs.existsSync(apiDir)) {
    ...
    addResult(
      ...
      mongodbUsageFound
        ? null
        : "MongoDB connection pattern not found in API routes",
    );
  }
}
...
function verifyConfigurationFiles() {
  ...
  if (fileExists(nextConfigPath)) {
    ...
    addResult(
      ...
      !fs.existsSync(pagesDir)
        ? null
        : "Pages directory found - may be using Pages Router",
    );
  }
}
...
function verifySecurityArchitecture() {
  ...
  if (fs.existsSync(apiDir)) {
    ...
    addResult(
      ...
      !authMiddlewareFound
        ? null
        : "Authentication patterns found in API routes",
    );
  }
...
function verifyTestingInfrastructure() {
  ...
  addResult(
    ...
    testFiles.length > 0
      ? `Found: ${testFiles.map((f) => path.relative(REPO_ROOT, f)).join(", ")}`
      : "No test files found",
  );
  ...
  if (packageJson && packageJson.scripts) {
    addResult(
      ...
      packageJson.scripts.test
        ? `Found: ${packageJson.scripts.test}`
...
function generateSummaryReport() {
  ...
  const accuracy =
    verificationResults.totalChecks > 0
      ? Math.round(
          (verificationResults.passed / verificationResults.totalChecks) * 100,
        )
      : 0;
  ...
  if (verificationResults.failed === 0) {
    ...
    console.log(
      "2. Update architecture-overview.md to match actual implementation",
    );
...
function main() {
  ...
  const resultsFile = path.join(
    REPO_ROOT,
    "architecture-verification-results.json",
  );
