...
function datesMatch(calculated, official) {
  return calculated.getFullYear() === official.getFullYear() &&
...
function test(description, calculated, official) {
  ...
  if (matches) {
    ...
    console.log(`   Calculated: ${result.calculated}, Official: ${result.official}`);
  } else {
    results.failed.push(result);
    console.log(`❌ FAIL: ${description}`);
    console.log(`   Calculated: ${result.calculated}, Official: ${result.official}`);
    console.log(`   Difference: ${Math.abs(calculated - official) / (1000 * 60 * 60 * 24)} days`);
  }
}
...
console.log('-'.repeat(80));
test('Easter 2024', calculateEaster(2024), OFFICIAL_CALENDAR_2024_2025['2024'].easter);
test('Easter 2025', calculateEaster(2025), OFFICIAL_CALENDAR_2024_2025['2025'].easter);
test('Easter 2026', calculateEaster(2026), OFFICIAL_CALENDAR_2024_2025['2026'].easter);
console.log();
...
console.log('-'.repeat(80));
test('Ash Wednesday 2024', calculateAshWednesday(2024), OFFICIAL_CALENDAR_2024_2025['2024'].ashWednesday);
test('Ash Wednesday 2025', calculateAshWednesday(2025), OFFICIAL_CALENDAR_2024_2025['2025'].ashWednesday);
test('Ash Wednesday 2026', calculateAshWednesday(2026), OFFICIAL_CALENDAR_2024_2025['2026'].ashWednesday);
console.log();
...
console.log('-'.repeat(80));
test('Advent 2024 Start', calculateAdventStart(2024), OFFICIAL_CALENDAR_2024_2025['2024'].advent.start);
test('Advent 2025 Start', calculateAdventStart(2025), OFFICIAL_CALENDAR_2024_2025['2025'].advent.start);
test('Advent 2026 Start', calculateAdventStart(2026), OFFICIAL_CALENDAR_2024_2025['2026'].advent.start);
console.log();
...
console.log(`${christmasSpecial === 'CHRISTMAS_DAY' ? '✅' : '❌'} Christmas Day 2024: ${christmasSpecial}`);
...
console.log(`${reformationSpecial === 'REFORMATION_SUNDAY' ? '✅' : '❌'} Reformation Sunday 2025: ${reformationSpecial}`);

// Test All Saints Day
const allSaints2025 = OFFICIAL_CALENDAR_2024_2025['2025'].allSaints;
const allSaintsSpecial = getSpecialDay(allSaints2025);
console.log(`${allSaintsSpecial === 'ALL_SAINTS_DAY' ? '✅' : '❌'} All Saints Day 2025: ${allSaintsSpecial}`);
...
console.log(`${christKingSpecial === 'CHRIST_THE_KING' ? '✅' : '❌'} Christ the King 2025: ${christKingSpecial}`);
...
console.log(`${thanksgivingSpecial === 'THANKSGIVING' ? '✅' : '❌'} Thanksgiving 2025: ${thanksgivingSpecial}`);
...
const criticalDates = [
  { date: '1/5/25', expected: 'CHRISTMAS', title: 'Second Sunday of Christmas' },
  { date: '1/12/25', expected: 'EPIPHANY', title: 'Baptism of our Lord' },
  { date: '3/2/25', expected: 'EPIPHANY', title: 'The Transfiguration of Our Lord' },
...
for (const { date, expected, title } of criticalDates) {
  ...
  if (season === expected) {
    console.log(`✅ ${date} (${title}): Season=${season}, Special=${special || 'None'}`);
  } else {
    console.log(`❌ ${date} (${title}): Expected ${expected}, Got ${season}, Special=${special || 'None'}`);
    results.failed.push({
...
test('Palm Sunday 2025 (Easter - 7)', palmSunday2025, OFFICIAL_CALENDAR_2024_2025['2025'].palmSunday);

// Check for Pentecost (Easter + 49 days)
const pentecost2025 = new Date(easter2025);
pentecost2025.setDate(easter2025.getDate() + 49);
test('Pentecost 2025 (Easter + 49)', pentecost2025, OFFICIAL_CALENDAR_2024_2025['2025'].pentecost);
...
test('Trinity Sunday 2025 (Pentecost + 7)', trinity2025, OFFICIAL_CALENDAR_2024_2025['2025'].trinity);
...
if (results.failed.length > 0) {
  ...
  results.failed.forEach(fail => {
    console.log(`  - ${fail.description}: Expected ${fail.official}, Got ${fail.calculated}`);
  });
  console.log();
  console.log('❌ VALIDATION FAILED - Foundation needs fixes before proceeding');
  process.exit(1);
} else {
