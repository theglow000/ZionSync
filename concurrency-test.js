...
class ConcurrencyControlTester {
  ...
  async testVersionFieldInclusion() {
    this.log('📋 Test 1: Verifying version field inclusion in components', 'test');
    ...
    const checks = [
      {
        ...
        check: () => signupSheet && signupSheet.includes('lastUpdated: serviceDetails[editingDate]?.lastUpdated'),
        file: 'SignupSheet.jsx'
      },
      {
        name: 'WorshipTeam sends lastUpdated with requests',
        check: () => worshipTeam && worshipTeam.includes('lastUpdated: serviceDetails[date]?.lastUpdated'),
        file: 'WorshipTeam.jsx'
      },
      {
        name: 'API route checks for version conflicts',
        check: () => apiRoute && apiRoute.includes('OPTIMISTIC CONCURRENCY CONTROL'),
    ...
    this.log(`📊 Version Field Test Results: ${passed}/${checks.length} checks passed`, 
  ...
  async testVersionMismatchDetection() {
    ...
    for (const scenario of scenarios) {
      ...
      const hasConflict = scenario.existing && scenario.incoming && 
      ...
      if (hasConflict === scenario.shouldConflict) {
        ...
        this.log(`  ❌ ${scenario.name}: FAILED - Expected ${scenario.shouldConflict ? 'conflict' : 'no conflict'}, got ${hasConflict ? 'conflict' : 'no conflict'}`, 'error');
        failed++;
      }
    }

    this.log(`📊 Version Detection Test Results: ${passed}/${scenarios.length} scenarios passed`, 
      failed === 0 ? 'success' : 'error');
    
    if (failed > 0) {
      throw new Error(`Version detection test failed: ${failed} scenarios failed`);
    }
  }
  ...
  async testConcurrentEditScenario() {
    ...
    const initialService = {
      ...
      elements: [
        ...
        { id: 'elem_3', type: 'reading', content: 'First Reading:', reference: '' }
      ]
    };
    ...
    const worshipEdit = {
      ...
      elements: [
        ...
        { id: 'elem_3', type: 'reading', content: 'First Reading:', reference: 'Isaiah 6:1-8' }
      ]
    };

    // Test the merge logic when worship team edit comes first
    this.log('🎭 Scenario: Worship team saves first, then pastor saves', 'info');
    ...
    const conflictDetected = afterWorshipSave.lastUpdated !== pastorEdit.lastUpdated;
    
    if (conflictDetected) {
      this.log('  ⚠️ Conflict detected: Pastor has outdated version', 'success');
      
      // Simulate the merge logic that should preserve worship team's selections
      const mergedResult = this.simulateMergeLogic(pastorEdit.elements, afterWorshipSave.elements);
      
      // Verify song selection is preserved
      const preservedSong = mergedResult.find(el => el.type === 'song_hymn' && el.selection?.title);
      if (preservedSong && preservedSong.selection.title === 'Amazing Grace') {
        this.log('  ✅ Song selection preserved during conflict resolution', 'success');
      } else {
        this.log('  ❌ Song selection lost during conflict resolution', 'error');
        throw new Error('Song selection not preserved during concurrent edit');
      }
      
      // Verify reading reference is preserved
      const preservedReading = mergedResult.find(el => el.type === 'reading' && el.reference === 'Isaiah 6:1-8');
      if (preservedReading) {
        this.log('  ✅ Reading reference preserved during conflict resolution', 'success');
      } else {
        this.log('  ❌ Reading reference lost during conflict resolution', 'error');
        throw new Error('Reading reference not preserved during concurrent edit');
      }
      
      // Verify pastor's new elements are included
      const addedReading = mergedResult.find(el => el.content === 'Second Reading:');
      if (addedReading) {
      ...
      throw new Error('Concurrency control not working - conflict not detected');
    }

    this.log('📊 Concurrent Edit Test: PASSED - All scenarios handled correctly', 'success');
  }
  ...
  simulateMergeLogic(newElements, existingElements) {
    ...
    return newElements.map(newElement => {
      if (newElement.type === 'song_hymn' || newElement.type === 'song_contemporary') {
        const prefix = newElement.content?.split(':')[0]?.trim()?.toLowerCase() || '';
  ...
  generateReport() {
    ...
    if (errorCount === 0) {
      ...
      this.log('✅ Version fields are properly included in requests', 'success');
      this.log('✅ Version conflicts are detected correctly', 'success');
      this.log('✅ Concurrent edits are merged safely', 'success');
      this.log('✅ Song selections and reading references are preserved during conflicts', 'success');
    } else {
      this.log('❌ CONCURRENCY TESTS FAILED!', 'error');
      this.log(`📊 Results: ${successCount} passed, ${errorCount} failed`, 'error');
    }
...
tester.testConcurrencyControl()
  .then(success => {
    console.log(success ? '✅ Concurrency control implementation verified!' : '❌ Concurrency control needs fixes');
