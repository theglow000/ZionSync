...
export async function POST(request) {
  try {
    ...
    if (existingService?.elements && body.elements) {
      console.log(
        "🔧 ENHANCED WORKFLOW: Detecting and handling orphaned songs",
      );
      ...
      if (orphanedSongs.length > 0) {
        ...
        console.log(
          `💾 BACKUP: Stored ${orphanedSongs.length} orphaned songs for potential recovery`,
        );
      }
      ...
      mergedElements = body.elements.map((newElement, index) => {
        if (
          ...
          if (existingSelection) {
            console.log(
              `🎵 PRESERVED: "${newPrefix}" → ${existingSelection.selection.title}`,
            );
      ...
      if (orphanedSongs.length > 0) {
        console.log(
          "🔄 SYNC: Updating service_songs collection to match new structure",
        );
        ...
        if (serviceSongs) {
          ...
          console.log(
            `🔄 SYNC: Updated service_songs with ${Object.keys(updatedSelections).length} songs (removed ${orphanedSongs.length} orphaned)`,
          );
        }
      }

      console.log(
        `✅ MERGE COMPLETE: Preserved ${preservedPrefixes.size} songs, orphaned ${orphanedSongs.length} songs`,
      );
    }
    ...
    const result = await db
      .collection("serviceDetails")
      .updateOne({ date: body.date }, { $set: updateDoc }, { upsert: true });
...
export async function PATCH(request) {
  try {
    ...
    if (action === "recover_orphaned") {
      ...
      const orphanedRecord = await db
        .collection("orphaned_songs")
        .findOne({ date }, { sort: { timestamp: -1 } });

      if (!orphanedRecord) {
        return NextResponse.json(
          { error: "No orphaned songs found for this date" },
          { status: 404 },
        );
      }
