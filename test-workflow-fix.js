...
const uri = process.env.MONGODB_URI || "mongodb+srv://theglenn:Gville2019@zionsync.ppyqd.mongodb.net/?retryWrites=true&w=majority&appName=ZionSync";
...
async function testWorkflowFix() {
  ...
  try {
    ...
    console.log('🧪 WORKFLOW FIX TEST: Starting order of worship preservation test...\n');
    ...
    const initialService = {
      date: testDate,
      type: "no_communion",
      content: "Prelude & Lighting of Candles\nOpening Hymn:\nConfession and Forgiveness\nHymn of the Day:\nSermon:\nSending Song:",
      elements: [
    ...
    const pastorEdit = {
      ...
      elements: [
        ...
        { id: "6", type: "reading", content: "First Reading:", reference: "Isaiah 42:1-9" }, // NEW
    ...
    const songsWithSelections = finalService.elements.filter(el => 
      (el.type === 'song_hymn' || el.type === 'song_contemporary') && el.selection
    );
    
    console.log('📊 RESULTS:');
    console.log(`   Total elements after pastor edit: ${finalService.elements.length}`);
    console.log(`   Songs with preserved selections: ${songsWithSelections.length}/3`);
    ...
    if (songsWithSelections.length === 3) {
      ...
      console.log('   Expected 3 songs with selections, got:', songsWithSelections.length);
    }
    ...
    console.log('   Testing what happens when pastor renames song positions...');
    console.log('   This should preserve "Hymn of the Day" but lose the others due to name mismatch');
