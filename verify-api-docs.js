...
function analyzeRouteFile(filePath) {
  try {
    ...
    let endpointPath = '/api/' + relativePath.replace(/route\.js$/, '').replace(/\\/g, '/').replace(/\/$/, '');
...
function parseDocumentation() {
  try {
    ...
    const endpointRegex = /####\s+(GET|POST|PUT|DELETE|PATCH)\s+(\/api\/[^\s\n]+)/g;
    let match;
...
function generateReport() {
  ...
  console.log(`   HTTP methods: ${Array.from(findings.httpMethods).join(', ')}`);
  ...
  Array.from(findings.collections).sort().forEach(collection => {
...
function main() {
  ...
  // Compare
  findings.discrepancies = compareEndpoints(findings.endpoints, documentedEndpoints);
...
module.exports = { analyzeRouteFile, findRouteFiles, parseDocumentation, compareEndpoints };
