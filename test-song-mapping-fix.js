...
async function testSongMappingFix() {
  ...
  try {
    ...
    console.log(
      "🎵 Step 2: Testing worship team song save with correct order...",
    );
      ...
      serviceDetails.elements.forEach((element, index) => {
        if (element.type === "song_hymn") {
          ...
          if (element.selection) {
            console.log(
              `     ✅ Selection: ${element.selection.title}${element.selection.number ? ` #${element.selection.number}` : ""}`,
            );
          } else {
    ...
    const expectedMappings = [
      {
        position: "Opening Hymn",
        expected: "What A Friend We Have in Jesus #742",
      },
      ...
      serviceDetails.elements.forEach((element) => {
        if (element.type === "song_hymn" && element.selection) {
          ...
          if (actual === expected.expected) {
            ...
            console.log(
              `❌ ${expected.position}: WRONG - Expected "${expected.expected}", got "${actual}"`,
            );
          }
          songIndex++;
        }
    ...
    if (mappingsCorrect === 3) {
      ...
      console.log(
        "   Worship team saves will now appear correctly in presentation team.",
      );
    } else {
