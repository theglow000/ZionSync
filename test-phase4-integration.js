...
async function testPhase4Integration() {
  ...
  try {
    ...
    console.log(
      "📝 Step 1: Pastor creates initial service with order of worship...",
    );
    ...
    const serviceSongsResponse = await fetch(
    ...
    if (!serviceSongsResponse.ok) {
      throw new Error(
        `Service-songs API failed: ${serviceSongsResponse.status}`,
      );
    }
    ...
    afterSongs.elements.forEach((element, index) => {
      if (element.type === "song_hymn" && element.selection) {
        ...
        if (actualSong === expectedSongs[index]) {
          ...
          console.log(
            `   ❌ Position ${index}: Expected "${expectedSongs[index]}", got "${actualSong}"`,
          );
    ...
    if (!serviceDetailsResponse.ok) {
      throw new Error(
        `Service-details API failed: ${serviceDetailsResponse.status}`,
      );
    }
    ...
    const finalService = await db
      .collection("serviceDetails")
      .findOne({ date: testDate });
    const songsAfterEdit = finalService.elements.filter(
      (e) => e.type === "song_hymn" && e.selection?.title,
    );

    console.log(
      `📊 Songs preserved after pastor edit: ${songsAfterEdit.length}/3`,
    );
    ...
    songsAfterEdit.forEach((song, index) => {
      ...
      if (song.selection.title === expectedTitle) {
        ...
        console.log(
          `   ❌ Expected "${expectedTitle}", got "${song.selection.title}"`,
        );
    ...
    console.log(
      conflictResponse.ok
        ? "✅ Concurrency conflict handled gracefully"
    ...
    if (passedTests === tests.length) {
      console.log(
        "🎉 SUCCESS: Cross-team fix works perfectly with Phase 4 improvements!",
      );
