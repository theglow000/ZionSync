...
async function testSongRemoval() {
  ...
  try {
    ...
    const removeMiddleResponse = await fetch(
      "http://localhost:3000/api/service-songs",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (!removeMiddleResponse.ok) {
      throw new Error(
        `Failed to remove middle song: ${removeMiddleResponse.status}`,
      );
    }
    ...
    afterRemove.elements.forEach((element, index) => {
      if (element.type === "song_hymn") {
        if (element.selection?.title) {
          console.log(
            `   ✅ Position ${index}: ${element.selection.title} (KEPT)`,
          );
        } else {
          console.log(
            `   📝 Position ${index}: ${element.content} (PENDING - cleared as expected)`,
          );
        }
      }
    });

    const songsAfterRemove = afterRemove.elements.filter(
      (e) => e.selection?.title,
    );
    console.log(
      `   📊 Songs remaining: ${songsAfterRemove.length}/3 (should be 2)`,
    );
    ...
    if (serviceSongs) {
      const validSongs = Object.entries(serviceSongs.selections).filter(
        ([key, value]) => value && value.title,
      ).length;
      console.log(`   📊 Valid songs in service-songs: ${validSongs}/3`);
      console.log(
        `   ${validSongs === 2 ? "✅" : "❌"} Presentation team sees correct number of songs`,
      );
    }
    ...
    const pastorResponse = await fetch(
    ...
    const finalSongsWithSelection = finalSongs.filter(
      (e) => e.selection?.title,
    );
    ...
    console.log(
      `   📊 Songs with selections: ${finalSongsWithSelection.length}`,
    );
    console.log(
      `   📊 Pending songs: ${finalSongs.length - finalSongsWithSelection.length}`,
    );
    ...
    const tests = [
      ...
      {
        name: "Presentation Team View",
        passed:
          serviceSongs &&
          Object.entries(serviceSongs.selections).filter(([k, v]) => v?.title)
            .length === 2,
      },
      {
        name: "Pastor Edit Preservation",
        passed: finalSongsWithSelection.length === 2,
      },
      {
        name: "Pending Slot Maintained",
        passed: finalSongs.length > finalSongsWithSelection.length,
      },
    ];
